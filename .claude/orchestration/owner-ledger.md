...
**Q8 (asked 2026-09-21 by plan-32)** — Q8 (asked 2026-09-21 by plan-32, on the owner's session's proposal) — the order in which parked tasks resume. While HEAD declares Development_Loci and Development_State_Rows without their files, no task gets a tree of its own: every task works in the one tree and each finalization holds it while the others park. When task 46 lands, the harness resumes parked tasks longest-parked first — 48 (parked 20:29, its replay claiming the machine), 49, 58 and 60 before task 22's fix (parked 21:06) — so 22 takes the tree only after them, and trees stay unavailable that long; the planner has no command that orders parked tasks. Provisional choice: the queue puts 46 then 22 first, and every build not yet started waits on both, so that it starts in a tree of its own after them instead of joining the one-tree queue; tasks already parked resume as the harness orders them. Basis: those two landings end the one-tree queue, and every later producing task then runs beside the others. Question: will you have the harness resume parked tasks in the planner's queue order (46, then 22's fix), rather than longest-parked first?

**Q9 (asked 2026-09-21 by plan-32)** — Q9 (asked 2026-09-21 by plan-32) — the tracked compiled object. tools/__pycache__/build.cpython-314.pyc is tracked although .gitignore already ignores it (__pycache__/ and *.pyc, line 3). Task 48 removed it from the working tree, but the finalizer refuses ignored paths, so the removal stands uncommitted (harmless: nothing reads the file, and Python regenerates it untracked). Three attempts have now failed. Provisional choice: leave the removal standing in the one tree and make no task of it, since the step is the finalizer's or yours. Basis: the finalizer is the harness's, and committing the removal of an ignored tracked path is the one change it refuses. Question: will you take the file out of the index yourself, or have the finalizer admit removing a tracked, ignored path?

**Q10 (asked 2026-09-21 by plan-33)** — Advancing the proof base (asked 2026-09-21 by plan-33). The active base (.build/complete-20260921a, adopted after the reboot) lags 15 theories of the commits since it, so every landing check re-executes all 33 recipes (#49's check: 1,785 of 1,800 theories reused, 181.94 s of recipes and host tests for a change to the plan alone). The recorded procedure advances the base (check --advance-base) before a long series of edits, and some thirty landings are queued. But a check run in a task's tree may leave its base in a tree that is removed after landing, and the one tree holds the uncommitted work of five parked tasks, so no session can advance the base on committed HEAD today. Provisional choice: no task advances the shared base; task 80 establishes where an advanced base would live and what it would save, and the base is advanced on committed HEAD once that is known and the one tree holds no uncommitted work. Basis: the recorded procedure (heapless checks on a fixed base) and the path-bound heaps. Question: should the finalizer advance the base on committed HEAD (after each landing, or when the rebuilt set passes a size), or is the planner to order it as a task?

**Q11 (asked 2026-09-22 by plan-33)** — Retaining verified receipts (asked 2026-09-22 by plan-33). Task 80 found that every verified receipt differs from HEAD on tools/execution_support.py since 955b34bb, so every landing check re-executes all recipes (about 180 s of checks of about 300 s) until the receipts are retained (incremental_check.py retain); advancing the base (Q10) saves only the proof phase, about 25 s. Provisional choice: a fix task retains the receipts once now, from an accepted check of committed HEAD, and the planner places another after a landing that changes many recipes' boundaries. Basis: the recorded procedure's retain command and the owner's single-digit-minute cycle. Question: should the finalizer instead retain the receipts of its accepted landing check at every commit?
