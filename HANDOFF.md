...
**#20** is outside every line, as #12 and #17 were: the replay's summary counts a run whose judgment could not be
...
- **A decision that cites rather than carries is the engine's measurement from the other side.** #9's closing
  ...
  locus the callee can descend. It is not a task yet — it wants #8's report, beside the store-search question.
- **What #8's report decides is three questions, not one.** Whether the positions renaming closed the engine's gap;
  ...
  evidence. One episode reads that report and decides them together.
...
repaired where it stands; the factoring, if it is ever one statement, belongs with the two native instances.
...
5. The refusal that an absent certified generation cannot tell from an unavailable input: an empty result and a
   ...
   absence, which the verdict's build carries, and as the replay's summary, which #20 repairs.
...
- **The harness's defects.** Seven are repaired: a stopped or stale background job no longer blocks a session for ever
  ...
  the record of who owns a working-tree change no longer takes a heredoc's words for filenames; a check that cannot
  run (an output directory that exists, or none named) is no longer counted as a failed check and goes back to the
  session at no cost in rounds; a path the active base was advanced with can no longer be set aside; and the guard
...
- **Six are left, for the orchestration, which repairs its own.** The harness counts the Isabelle runs of the
  ...
  `tools/native_answers.py:380`, and a whole sixteen-answer replay was discarded and rerun. The rule now in Decisions
  binds what a *brief* says, not what the finalizer launches, so the finalizer must hold the sum: refuse to start a
  check while a base-advancing check holds the machine (the harness already refuses commands for that reason), or run
  its replay at `--workers 1`. And a task's finalization holds DECISIONS.md for its whole length, so no other task's
