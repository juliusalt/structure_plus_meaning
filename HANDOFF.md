...
- CHECK ad (impl-27's) proved all 1791 theories (392 s) but the seed and machinery exports did not compile:
  ...
  `declare ..._def [code_unfold]` and the install copy lost it. FIXED in `theories/Native_Collection_Programs.thy`
  and in `.build/impl27/make_install.py` (it now re-inserts both declarations; its later part reads theories/ and
  is not re-runnable after install, harmless).
- `adopt --proof .build/check-20260919ad/proof`, CHECK ae `--advance-base` (87 theories, 159 s): every stage exit 0;
  seed changed only `presentation-loop`; machinery all 5 words (its state = the loop's notions: 65 roots,
  313 names, 355 entities, 65 residuals, 36 ready); steering/source-development changed the stages with a nonempty
  question. PROBES (`.build/impl28/steer`, old base ac vs new ad): steering decisions identical (full [0], first
  [0,8], empty None, choice 0, steered results and source admissions equal); (`.build/impl28/counts`) native
  selection = HOL ready list on seed (10/10) and machinery (36/65). Words recorded (`.build/impl14/record_words.py`),
  `adopt --proof .build/check-20260919ae/proof` (ACTIVE BASE: ae), CONFIRMING CHECK af ACCEPTED (6 recipes equal,
  177+35 tests) and RETAINED.
- REPLAY (`.build/impl28/replay-a`): 16 replayed, 15 reconstructed with every word equal, walk adopted, none differing.
...
- LATER (provisional, Q7 step 2): the verdict of a kind as a native definition. Design: the native
  ...
  verdict = persistence/additions/demanded/support (collection notions, positive only) + closedness (reachability
  closure from the roots over mentions, like settlement) + roots; contract against
  `development_verdict_accepted (development_constant_verdict ...)`; then the native judgment consumes it; request
  construction and contract decisions over the same rows next.

## impl-27 session (2026-09-19) — its batch validated and committed by impl-28

- DESIGN (provisional residual, recorded in the plan section to write): readiness is a native program over ROWS
  (key, structural status: answered = `Payload []`, open = `Pair [] []`, decompositions = lists of premise keys);
  each question CANDIDATE is `Pair cone row`, where the cone is the table of the rows its settledness reads (the
  answered problems reachable from its premises through answered problems; transitive closure of answered-premise
  edges computed once, `development_readiness_closure`); the question's subject is empty (`Finite_Payload []`).
  `ready(Pair X (Pair C (Pair k (Pair OPEN Hs)))) :- every(Pair C Hs)`; settled/search/answered/some/all over C.
  Candidates: `.build/impl27/cand/P27_{Native_Readiness,Native_Selection,Selection_Question,Debug}.thy` (with
  impl-25's `.build/impl25/cand` for the collection programs, subject questions, P25 closure); all certified by a
  serial probe (`--parallel-proofs 0`, loaded).
- MEASURED (probe, native vs old HOL-reflected, equal results): seed 0.059 s (0.020), machinery 0.647 s (0.203;
  was 89.6 s with the premise table as the question's subject, >900 s with impl-24's program), free 16/32/64/128:
  0.08/0.18/0.56/2.6 s, chain 8/16/32 (half answered): 0.07/0.24/3.0 s. Stage attribution of the rejected
  subject-table design: observations 90.8 s, generation 4.3 s (61 scope rules x components of the subject).
- INSTALLED IN theories/ (uncommitted), generated by `python3 .build/impl27/make_install.py` into
  `.build/impl27/install/` and copied: Native_Collection_Programs (new; ROOT after
  Factor_Development_Criterion_Sources), Factor_List_Profiles (context_list_rule_relation), Factor_Development_Cases
  and _Criterion_Sources (program and standalone conditions), Factor_Finite_Development_Questions (scope program,
  subject question; the old question is the subject question over `Finite_Payload []`), Factor_Development_Steering,
  Filtered_Native_Questions (one proof), Development_Native_Readiness/_Selection (rewritten), Development_Requests
  (selection = native question; `development_admitted_problems`, `development_packet_problems`),
  Development_Successor/_Machinery (consumers), Finite_Demanded_Closures + Factor_Demanded_Program_Calls (P25
  traversal and `finite_program_call_closure` appended), Factor_Workflow_Stage (stage demand = call closure of its
  requests), Factor_Workflow_Execution_Sharing + Keyed_Native_Evaluation (code equations).
- RUNNING: `incremental_check.py check --advance-base --output .build/check-20260919ad` (log
  `.build/check-20260919ad.out`). Near-full rebuild (Factor_List_Profiles has ~964 dependents). EXPECTED: every
  native-question recipe word changes (scope program; native selection) -> re-record with
  `.build/impl14/record_words.py CHECK RECIPE`, `adopt --proof .build/check-20260919ad/proof`, confirming check,
  retain, `replay_development_answers.py --rerecord`; THEORY_MAP rows (impl-25's `.build/impl25/theory_map.py` as a
  start), plan section + REASONING_REUSE section (drafts `.build/impl25/docs/*` to adapt), commit + push.
- On a proof failure in the check: fix in theories/ AND in the candidates/install generator.
- PARKED at the context limit while check ad runs (python pid 648818; `.build/check-20260919ad.out`). ALSO DONE,
  uncommitted: THEORY_MAP.md rows applied (`.build/impl27/theory_map.py`, new Native_Collection_Programs row).
  READY TO APPEND: `.build/impl27/docs/plan-section.md` (to native_control_plan.md; fill EVIDENCE with check ad,
  re-recorded recipes, confirming check, replay) and `.build/impl27/docs/rr-section.md` (to REASONING_REUSE.md);
  commit message draft `.build/impl27/commit.txt` (fill VALIDATION; no attribution). Do NOT commit
  `tools/__pycache__/*.pyc` or `.claude/orchestration/*` (not this batch's). Then `git push origin main`.
- NEXT BATCH after the commit (Q7 order): the loop's remaining reflected decisions are the constant problems'
  contract questions (filtered questions on HOL-computed demands); making them native needs entity kinds as
  structure (task 2 of "Structure is explicit"), then the verdict of a kind and request construction natively.
  Scratch to remove after commit: `.build/impl24`, `.build/impl25`, `.build/impl26`, probe dirs `.build/probe-impl27-*`.

## impl-26 session (2026-09-19, read first) — theories/ untouched, nothing committed, nothing of mine running

- FIXED (impl-25 candidates, `.build/impl25/cand`): `P25_Demanded_Traversal` (corollary by `trans[OF within exact]`),
  `P25_Program_Calls` (`rule ext` + `unfolding read[symmetric]`); serial probe `.build/probe-impl26-a` LOADED.
  `P25_Debug` parenthesized `u=(0<...)`. Its run (`.build/probe-impl26-b`): native SEED selection 0.111 s (old 0.020)
  and EQUAL to the old selection; native MACHINERY selection (61 problems) did not finish in 900 s (old 0.223 s).
- ATTRIBUTION (`.build/impl26/{attrib,attrib3,size,profile}`, synthetic n problems, no deps): calls 161/577/1249/2177
  at n=8/16/24/32 (~2n^2: per-candidate walks over Open and the table, each call carrying the whole table);
  closure 0.005/0.08/0.70/3.4 s, evaluation 0.005/0.05/0.23/0.71, certificates 0.007/0.10/0.68/2.8 (~n^5);
  observation word bits 0.31M/1.9M/6.9M at 8/16/24 (~n^3); machinery generation of the scope program 4.7 s
  (61 rules x every component of the subject). ML profile (`.build/probe-impl26-i/probe.log`): 70-85% deep term
  equality/compare: fset union (`set xs ∪ A = fold insert`) dedupes by deep equality, calls share the table first.
- `.build/impl26/cand/Keyed_Demanded_Closures.thy` (RBT frontier; LOADS, certified) is NOT effective: a polymorphic
  `[code]` lemma for `finite_program_call_readings` is silently dropped (needs a monomorphic use, e.g. in
  Keyed_Native_Evaluation's stage code), and the union dedupe of `S |∪| T`/`A |∪| rows` remains; not needed for the
  redesign below — keep as a recorded engine problem (quadratic fset unions with deep equality).
- DECISION (provisional, a residual; record in the plan section): ROUTE R+ — candidates are the table ROWS, not keys:
  row = (key, status, decompositions), status structural (ANSWERED = `Payload []`, OPEN = `Pair [] []`), no A/Open
  lists, no member site; `ready(Pair P (Pair k (Pair OPEN Hs))) :- every(Pair P Hs)` (no lookup for the candidate);
  `settled(Pair P q) :- settled_search(Pair P (Pair q P))`, search element `answered(Pair P (Pair ANSWERED Hs)) :-
  some(Pair P Hs)`; SUBJECT P = the premise table (rows of the problems some decomposition premises; closed; the
  least context readiness reads). Contract: ready on a row of the table <-> development_ready (rows come from the
  scope, built from the same table); settledness restricted to the premise table equals it on premise keys.
  Expected machinery calls ~400 (only 3 dependency rows walk). Rewrite P24_Native_Readiness (sites/rules/invariant/
  contracts/presentation), P24_Native_Selection (readiness_presents for rows+status), P24_Selection_Question
  (rows as candidates, premise-table subject, selected_ready/independent); then P25_Debug on seed+machinery+scale.
- THEN the install as impl-25 planned (make_install.py; Factor_List_Profiles; P25 traversal lemmas into
  Finite_Demanded_Closures; closure into Factor_Demanded_Program_Calls; stage demand switch in Factor_Workflow_Stage
  + Factor_Workflow_Execution_Sharing + Keyed_Native_Evaluation (3 code eqs each: `finite_program_call_closure P R`);
  ROOT; THEORY_MAP; check --advance-base; re-record; adopt; confirm; retain; replay; docs; commit+push).
- Probe dirs to remove: `.build/probe-impl26-{a..i}`, `.build/probe-impl25-*`. Probe `.build/probe-impl26-f`
  (old list closure on the machinery) may still run until its 900 s timeout; harmless.

## impl-25 session (2026-09-19, read first) — PARKED at the context limit; theories/ untouched, nothing committed

- CANDIDATES now in `.build/impl25/cand/` (impl-24's, repaired; `.build/impl24/cand` is superseded): every proof
  of Native_Collection_Programs .. P24_Selection_Question LOADS (probe `.build/probe-impl25-d`, loaded true, 7.5 s).
  Repairs: explicit proofs instead of stalling `force`; `unfolding <locale>_def` for the collection-locale
  interpretations; `native_rule_support`/`native_step` instead of simp on decoded schemas; `readiness_members.exact
  [THEN iffD1/iffD2]` (simp turns `[1]` into `[Suc 0]` in sites, so never apply site lemmas by simp); `[0::nat]` in an
  obtain; no fact named `premises`; polymorphic rule values `native_member_here`/`native_every_nil` get `[code_unfold]`
  (SML value restriction). DEDUPLICATION: `native_every` restated `context_list_profile` at native coordinates;
  now one notion `context_list_rule_relation` (P25_Context_List; install copy `.build/impl25/install/
  Factor_List_Profiles.thy` puts it into Factor_List_Profiles and derives `context_list_profile.sound/complete/
  exact/lists` from it via `relation_equation`; 964 dependents rebuild), and `native_every_program` interprets it.
- FINDING (P25_Debug, value [code]): the native selection question is built (10 seed candidates generated) but
  ADMISSION IS NONE on seed and machinery: the readiness condition's stage returns None, because
  `workflow_stage_demand` = `finite_program_demanded_calls P T R` reads applications only at calls inside the
  TERM DEMAND (subterms of the requested arguments); readiness (and every context-passing program, the library's
  context lists included) premises NEW pairs of components, so the demand is not closed. FIX WRITTEN (candidates):
  `P25_Demanded_Traversal` (traversal correct without a universe `finite_demanded_readings_closed`; a closed
  universe containing the roots bounds it `finite_demanded_readings_within(_exact)` via
  `while_option_commute_invariant`) and `P25_Program_Calls` (`finite_program_call_closure P R`: pure demanded
  closure, None-safe; `_requests`, `_closed`, `_term` = old demanded calls when the term demand is closed, so
  existing stages are unchanged; `_ready`, `_available_ready`). NOT YET PROBED.
- RUNNING at park: `.build/probe-impl25-f` (P25_Debug: quick_and_dirty code equation making
  `finite_program_demanded_calls` compute the closure; value [code] conjuncts + compare, then code_reflect timings
  `PROBE old/native seed/machinery selection seconds ... result`; summary `.build/impl25/probe-f.out`, log
  `.build/probe-impl25-f/probe.log`). P24_Measure's value timings were meaningless (value prints asynchronously).
- NEXT: read probe-f (expect conjuncts all True and compare True with the closure; note times; machinery has 61
  problems). If admission works: install = make_install.py output + install/Factor_List_Profiles + append
  P25_Demanded_Traversal's content to Finite_Demanded_Closures (or new theory) + new theory
  Factor_Program_Call_Closures (P25_Program_Calls) + switch `workflow_stage_demand` (Factor_Workflow_Stage;
  `workflow_stage_demand_ready` via `finite_program_call_closure_ready[OF ready workflow_stage_requests_demanded]`)
  and the code equations in Factor_Workflow_Execution_Sharing and Keyed_Native_Evaluation from
  `finite_program_demanded_calls P (fset_of_list (map (Finite_Pair x) ys)) R` to `finite_program_call_closure P R`;
  ROOT (Native_Collection_Programs after Factor_Development_Criterion_Sources; call closures after
  Factor_Demanded_Program_Calls); THEORY_MAP via `.build/impl25/theory_map.py` (+ rows for the new theories);
  docs drafts `.build/impl25/docs/{plan-section,rr-section}.md` (add the demand correction row + evidence);
  `incremental_check.py check --advance-base` (near-full rebuild), re-record native-question recipe words
  (`.build/impl14/record_words.py CHECK RECIPE`), adopt, confirm, retain, replay --rerecord, commit+push.
  Open: generation of the scope program costs (clauses x components of the subject).

## impl-24 session (2026-09-19, read first) — PARKED at the context limit; theories/ untouched, nothing committed

- impl-23 left no handoff; its drafts (`.build/impl23/real/`) are superseded by `.build/impl24/cand/`.
- CANDIDATES (`.build/impl24/cand/`, probe with `tools/probe_theories.py --candidates .build/impl24/cand`):
  `Native_Collection_Programs` (impl-23's native rule families, PROOFS REWORKED: `finite_native_rule_fields`,
  `native_rule_variables`, `native_rule_support`, locale `native_rule_family` with `native_step`; member/every/
  some/keyed-search contracts; `finite_rule_program` lemmas). Last forked probe (`.build/probe-impl24-a`) showed NO
  failure in it but then STALLED (>90 s, killed): some proof does not return -- suspects `force` in
  `finite_rule_program_interface`/`_clause`, or an `auto` in an `unfold`. RUNNING at park: serial probe of that
  theory alone, `.build/probe-impl24-b` (summary `.build/impl24/probe-b.out`, 120 s timeout; the log's last
  statement names the stalling proof). RESULT: serial probe timed out (120 s) right after `finite_rule_program`'s
  definition, i.e. in `finite_rule_program_clause` (the `force` with `fset_of_list.rep_eq`): replace it by an
  explicit proof like `finite_rule_program_interface` (have the clause set as an image of the listed rows, then
  membership). Then probe all (forked), then `--parallel-proofs 0` to certify.
  `P24_Program_Condition` (-> install into Factor_Development_Cases/Criterion_Sources): `finite_program_condition Q e`
  (+`_exact`, `_total`), `finite_standalone_condition R e` (union with the guard program; `_total`, `_exact`),
  `finite_ground_condition_program`, `development_scope_condition_program`. `P24_Native_Readiness` (readiness as a
  finite rule program at sites `(Some [],[1..8])`, `native_settled_exact`, `native_ready_exact`, finite presentation
  `finite_readiness_table/subject` + decode lemmas, `native_readiness_condition` + `_total`/`_exact`).
  `P24_Native_Selection` (readiness_presents over FORMED keys, `readiness_presents_formed`, `native_development_ready`
  at site `readiness_ready`). `P24_Subject_Questions` (scope rules `Pair X y :- guard(X)` over the guard's universal
  recognizer: `finite_scope_program_meaning`, `finite_scope_source(_meaning,_total)`, `finite_subject_question`
  + `_fields`/`_conditions`). `P24_Selection_Question` (keys = first positions via `value_reference_index`,
  `development_readiness_rows/subject/scope_closed`, `development_readiness_presents`,
  `development_native_selection_question` = subject question over the readiness subject, candidates = position
  indices, condition = native readiness; `development_native_selected_ready`, `_independent`). `P24_Measure`
  (value [code] timings: old vs native seed selection, native machinery selection; compares selected lists).
  None of P24_Program_Condition..P24_Measure has loaded yet (blocked behind the collection theory).
- INSTALL PREPARED (`.build/impl24/install/`): Factor_Development_Cases (existing condition moved in; program
  condition; scope condition through it), Factor_Development_Criterion_Sources (standalone condition; imports
  Factor_Finite_System_Unions), Factor_Finite_Development_Questions (scope program; `finite_development_source` :=
  scope source; `finite_development_question xs facets` := subject question over `Finite_Payload []`; imports
  Native_Collection_Programs). `python3 .build/impl24/make_install.py` regenerates from the candidates:
  Native_Collection_Programs, Development_Native_Readiness, Development_Native_Selection (+ the presentation
  section), Development_Requests (`development_selection_question` := native; text "presented as above" to fix),
  Factor_Development_Steering (equation lemma with `finite_development_source`). `.build/impl24/theory_map.py`
  rewrites THEORY_MAP rows. ROOT: add `Native_Collection_Programs` after `Factor_Development_Criterion_Sources`.
- NEXT: finish probes; copy install into theories/, ROOT, THEORY_MAP; `incremental_check.py check --advance-base
  --output .build/check-20260919ad` (~90 theories): EVERY native-question recipe word changes (generator is now the
  scope program; seed/machinery selection is native) -> re-record with `.build/impl14/record_words.py CHECK RECIPE`,
  adopt, confirming check, retain, replay answers `--rerecord`; plan section + RR section; commit + push.
  Open design points: generation cost of the scope program is (clauses x components of the subject); the evidence
  of native readiness (certificates) carries the whole subject in every call -- measure word size on the machinery.
