...
amount to for the stages and what they leave open.
...
present them; an answer is judged by the difference it makes, with the verdict's local contract, and a refusal
derives its repair; admitted answers are generations whose causes are certified under the first loop's policy, and
are published as transactions against the published state; succession transports readiness, independent answers are
admitted concurrently, and absence readings are re-evaluated; the loop's decisions are admitted generations; the
...
The second direction's order, which supersedes the first order:
