...
section \<open>Partial inverses from an exact domain and an injective decoder\<close>

lemma partial_inverse_exact:
  fixes encode :: "'a \<Rightarrow> 'b option" and decode :: "'b \<Rightarrow> 'a"
  assumes domain: "\<And>x. encode x=None \<longleftrightarrow> \<not>P x"
    and sound: "\<And>x y. encode x=Some y \<Longrightarrow> decode y=x"
    and injective: "inj decode"
  shows "encode x=Some y \<longleftrightarrow> P x \<and> decode y=x"
proof (cases "encode x")
  case None
  then show ?thesis using domain[of x] by simp
next
  case (Some z)
  have accepted: "P x" using domain[of x] Some by simp
  have decoded: "decode z=x" by (rule sound[OF Some])
  show ?thesis using Some accepted decoded injective
