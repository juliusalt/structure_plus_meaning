theory Development_Given_Declarations
  imports Factor_Resolution_Views Development_Given_Program Factor_Bag_Presentations Factor_Binder_Admission
begin
...
text \<open>
  ...
  readers (@{const finite_rooted_given_readers}) unchanged. The union of records and the transfer of a record's
  discharge from its notion's system to a program sharing its definitions are stated here once
  (@{text declarations_list_discharged}, @{text declarations_shared_discharged}); the given's record, R6's pieces
  with R6b's (@{text given_declarations}), is carried to the given's programs in
...
section \<open>A union of records, and its discharge\<close>
...
section \<open>A record discharged at a program sharing its system's definitions\<close>

text \<open>
  A record's obligations read the meaning at its producers' sites, its consumers' own sites and its sockets' callees
  alone; a consumer's producer site only names the correspondence. A program Q agreeing with the record's system P on
  their common definitions means what P means at every site both hold, and nothing at a site it does not hold: a
  producer or a consumer there is vacuous, and a socket whose clause calls a site Q does not hold has no true instance
  at Q. So the record is discharged at Q, whatever Q holds beyond it, where P holds the sites the obligations read.
  @{text declarations_agree_discharged} asks more: every declared site in a dependency-closed agreement set.
\<close>
...
proof -
  ...
  have eq: "(d,x) \<in> ?N \<longleftrightarrow> (d,x) \<in> ?M" if "d \<in> system_definitions P" "d \<in> system_definitions Q" for d x
    using positive_meaning_shared_definitions[OF Pf Qf agree, of d x] that by blast
  have sub: "(d,x) \<in> ?M" if "d \<in> system_definitions P" "(d,x) \<in> ?N" for d x
    using eq[OF that(1) positive_meaning_site[OF that(2)]] that(2) by blast
  have dF: "declarations_formed D"
    and dP: "\<forall>d V hs. (d,V,hs) |\<in>| declared_producers D \<longrightarrow> producer_discharged ?M d V hs (corr d)"
    and dC: "\<forall>d e V i. (d,e,V,i) |\<in>| declared_consumers D \<longrightarrow> consumer_discharged ?M e V (corr d i)"
    and dS: "\<forall>e S s keep Vp Vh. (e,S,s,keep,Vp,Vh) |\<in>| declared_sockets D \<longrightarrow> socket_discharged ?M S s keep Vp Vh"
  ...
  next
    fix d V hs assume m: "(d,V,hs) |\<in>| declared_producers D"
    have ds: "d \<in> system_definitions P" by (rule producers_in[OF m])
    have src: "producer_discharged ?M d V hs (corr d)" using dP m by blast
    show "producer_discharged ?N d V hs (corr d)"
      using src sub[OF ds] unfolding producer_discharged_def by blast
  next
    fix d e V i assume m: "(d,e,V,i) |\<in>| declared_consumers D"
    have es: "e \<in> system_definitions P" by (rule consumers_in[OF m])
    have src: "consumer_discharged ?M e V (corr d i)" using dC m by blast
    show "consumer_discharged ?N e V (corr d i)"
    proof (cases "e \<in> system_definitions Q")
      case True
      then show ?thesis using src eq[OF es True] unfolding consumer_discharged_def by simp
    next
      case False
  ...
  next
    fix e S s keep Vp Vh assume m: "(e,S,s,keep,Vp,Vh) |\<in>| declared_sockets D"
    have deps: "schema_dependencies (decode_finite_schema S) \<subseteq> system_definitions P" by (rule sockets_in[OF m])
    ...
    have eqs: "(d,x) \<in> ?N \<longleftrightarrow> (d,x) \<in> ?M"
      ...
      using eq[OF subsetD[OF deps that(1)] that(2)] .
