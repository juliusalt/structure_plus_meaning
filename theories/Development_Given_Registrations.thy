...
proof -
  show "finite_registration_complete finite_given_readers n bound_witness_registration"
    by (rule bound_witness_registration_complete)
      (simp_all only: finite_given_readers_exact given_readers_meanings)
  show "finite_registration_complete finite_given_readers n (additions_witness_registration 392 391)"
    by (rule additions_witness_registration_complete[where element_site=390])
      (simp_all only: finite_given_readers_exact given_readers_meanings given_readers_listing)
...
proof -
  have closure: "systems_agree_on package_closure_admission_system guard_readers_system
      (system_definitions package_closure_admission_system)"
    by (rule whole_agreement_transitive[OF given_reader_agreements(2) additions_guard_agreement])
  have at77: "((77,c),T)\<in>system_clauses guard_readers_system \<longleftrightarrow> c=0 \<and> T=package_closure_admission_schema" for c T
  proof -
    ...
    then have "((77,c),T)\<in>system_clauses package_closure_admission_system \<longleftrightarrow>
      ...
      using closure unfolding systems_agree_on_def by blast
    then show ?thesis by simp
  qed
  have at392: "((392,c),T)\<in>system_clauses guard_readers_system \<longleftrightarrow> c=0 \<and> T=package_additions_schema 391" for c T
  proof -
  ...
  show "((77,c),S) |\<in>| finite_system_clauses finite_given_readers \<longleftrightarrow>
      c=0 \<and> finite_registration_matches 77 S bound_witness_registration"
    by (simp only: finite_given_readers_single_clause[OF at77] finite_registration_matches_def
  ...
  show "((392,c),S) |\<in>| finite_system_clauses finite_given_readers \<longleftrightarrow>
      c=0 \<and> finite_registration_matches 392 S (additions_witness_registration 392 391)"
    by (simp only: finite_given_readers_single_clause[OF at392] finite_registration_matches_def
...
text \<open>
  ...
  the registered forms ask every registration of the list complete at the program, which 525's is not asked to be
  here.
\<close>

lemmas given_readers_resolution_refutation_exact =
  finite_complete_resolution_refutation_exact[OF _ given_readers_construction_complete]

lemmas given_readers_verdict_exact =
  finite_complete_verdict_exact[OF _ given_readers_construction_complete]
...
proof -
  show merge: "finite_registration_complete P n merge_witness_registration"
    by (rule merge_witness_registration_complete) (simp_all only: presented request_readers_meanings)
  show bound: "finite_registration_complete P n bound_witness_registration"
    by (rule bound_witness_registration_complete) (simp_all only: presented request_readers_meanings)
