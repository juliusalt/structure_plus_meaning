theory Factor_Authority_Independence
  imports Factor_Authority_Programs Factor_Generation_Causes Factor_Base_Programs
begin
...
text \<open>
  ...
  Adoption can accept a generation whose recorded cause is invalid. A complete
  closed generation with a retained valid base certificate can be refused by
