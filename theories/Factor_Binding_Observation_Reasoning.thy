...
proof -
  obtain V Q where inst: "schema_instance binding_observation_pair_schema V t Q"
  ...
  have head: "schema_call_formed binding_observation_program 347 t"
    using formed by (simp add: binding_observation_call)
  have clause: "((347,0),binding_observation_pair_schema)\<in>system_clauses binding_observation_program"
    by (simp add: binding_observation_clauses_def)
  show ?thesis by (rule positive_clause_rule_instance[OF clause head rule])
qed
