theory Factor_Inference_Specialization_Total
  imports Factor_Inference_Specialization_Contracts Factor_Specialization_Binding_Total
...
proof -
  obtain report bs RIs RKs where binding: "distinct bs"
    "specialization_binding_at E pu pr d c F v r G w t report (positioned_binding_rows_term bs) RIs RKs"
    "graph_node_inputs_at E ({} :: (local_address option definition_site \<times>
      local_address option definition_site) set) (Schema_Inference (fst d,c) (fset_of_list bs))"
    using specialization_binding_total[OF actual] by blast
  obtain P where package: "native_package_at E pu pr P"
...
proof -
  obtain z where admitted: "(294,z)\<in>positive_meaning clause_specialization_reading_system"
    using clause_specialization_reading_inhabited by blast
  obtain E pu pr d c F f v r G g w t where actual:
    "schema_clause_specialization_at E pu pr d c F v r G w t"
    "environment_value_presents F f" "environment_value_presents G g"
    using admitted by (auto simp only: clause_specialization_reading_exact clause_specialization_reading_result_def)
  show ?thesis using inference_specialization_reading_total[OF actual] by blast
qed
...
text \<open>
  ...
  inhabited result uses the already constructed native specialization. These
  semantic proofs supply actual environment and reading witnesses; an executed
  explicit finite example still requires exporting and checking such witnesses.
  Neither theorem asserts whole mathematical-proof admission.
\<close>
