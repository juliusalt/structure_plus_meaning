...
text \<open>
  ...
  one registration's; the construction from complete, distinct registrations complete; and relocation of a
...
text \<open>
  ...
  registration's collection value, and a construction from complete distinct registrations is complete.
\<close>
...
theorem finite_collection_construction_complete:
  assumes distinct: "finite_registrations_distinct Rs"
    and complete: "\<And>R. R \<in> set Rs \<Longrightarrow> finite_registration_complete P n R"
...
proof (intro allI impI)
  ...
  then obtain R where R: "R \<in> set Rs" "finite_registration_matches d S R" "registration_variable R = a" by blast
  have d: "registration_site R = d" and S: "registration_schema R = S"
  ...
  proof -
    fix B show "witness_value (finite_collection_construction Rs n) P d S B a = finite_registration_value P n R B"
      using finite_collection_construction_value_at[OF distinct R(1), of n P B] unfolding d S R(3) .
  qed
...
text \<open>
  At the construction of a list of registrations, each complete and no two naming one variable of one clause, the
  forms are exact (@{text finite_collection_construction_complete}).
\<close>
