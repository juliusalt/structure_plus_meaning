theory Factor_Native_Incidence
  imports Factor_Material_Meaning Factor_Native_Meaning Factor_Application_Encoding Factor_Closed_Native_Definitions
begin
...
lemma incidence_definition_no_dependencies:
  ...
  using native_definition_edges_at[OF incidence_definition_at, of e]
  by (simp add: schema_dependencies_def native_incidence_schema_def rel_ran_def)

lemma incidence_closed_definition_graph:
  ...
  "native_definition_graph incidence_environment {(None,[Suc 0])} =
    {((None,[Suc 0]),Pattern_Variable [4],{([7],native_incidence_schema)})}"
  using native_closed_definition_graph[OF incidence_environment_formed,
    where G="{((None,[Suc 0]),Pattern_Variable [4],{([7],native_incidence_schema)})}"]
    incidence_definition_at
  by (auto simp: rel_dom_def schema_dependencies_def native_incidence_schema_def rel_ran_def)

lemmas incidence_definition_sites = incidence_closed_definition_graph(1)
lemmas incidence_package_formed = incidence_closed_definition_graph(2)

lemma incidence_program_projection:
  "native_program incidence_environment {(None,[Suc 0])} = native_incidence_program"
  by (auto simp: native_program_def incidence_closed_definition_graph(3) native_incidence_program_def)
