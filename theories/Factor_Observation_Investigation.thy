theory Factor_Observation_Investigation
  imports Factor_Observation_Results Finite_Observation_Contracts Native_Execution_Refinements
begin
...
definition observation_investigation_decision :: "nat \<Rightarrow> nat \<Rightarrow> bool" where
  "observation_investigation_decision f n=observation_subject_operation f (observation_investigation_output n)"

lemma observation_investigation_decision_code [code]:
  "observation_investigation_decision f c=(if f=0 then c=0 else c<3)"
  by (simp only: observation_investigation_decision_def observation_subject_operation_def observation_encoded_profile observation_profile_result_lists)
...
lemma observation_subject_decision_at:
  "observation_subject_operation f (observation_investigation_output c)=observation_investigation_decision f c"
  by (simp only: observation_investigation_decision_def)

definition observation_candidate_subjects where
  "observation_candidate_subjects=fimage (\<lambda>c. (c,observation_investigation_output c)) {|0::nat,1,2,3|}"

definition observation_operation_subjects where
  "observation_operation_subjects=fimage (\<lambda>f. (f,observation_subject_operation f)) {|0::nat,1|}"

definition observation_value_subjects where
  "observation_value_subjects=fimage (\<lambda>w. (w,w=1)) {|0::nat,1|}"

lemma observation_subject_maps_formed:
  "finite_observation_subjects_formed observation_candidate_subjects observation_operation_subjects observation_value_subjects"
  unfolding observation_candidate_subjects_def observation_operation_subjects_def observation_value_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma observation_observations_derived:
  "fset_of_list observation_investigation_observations=
    finite_derived_observations observation_candidate_subjects observation_operation_subjects
      observation_value_subjects (\<lambda>operation subject value. value=operation subject)"
  unfolding observation_candidate_subjects_def observation_operation_subjects_def observation_value_subjects_def
  apply (rule finite_table_derived_from_function_graphs)
  by (simp only: fset_of_list.rep_eq observation_investigation_observations_def indexed_value_rows_member observation_subject_decision_at;
    auto split: if_splits)

theorem observation_observation_at_subject:
  assumes "(c,subject) |\<in>| observation_candidate_subjects"
    "(f,operation) |\<in>| observation_operation_subjects"
    "(w,value) |\<in>| observation_value_subjects"
  shows "(f,c,w)\<in>set observation_investigation_observations \<longleftrightarrow> value=operation subject"
  using finite_derived_observation_at_subject[OF observation_subject_maps_formed assms, where P="\<lambda>operation subject value. value=operation subject"]
  by (simp only: observation_observations_derived[symmetric] fset_of_list.rep_eq)

theorem observation_comparison_at_subject:
  assumes "(c,subject) |\<in>| observation_candidate_subjects"
    "(d,other) |\<in>| observation_candidate_subjects"
  shows "(c,d)\<in>set observation_investigation_relation \<longleftrightarrow> set subject=set other"
  using assms by (auto simp: observation_candidate_subjects_def fimage.rep_eq observation_investigation_relation_exact)

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm observation_investigation_def},
   equation = @{thm observation_observations_derived},
   formation = @{thm observation_subject_maps_formed},
   observation = @{thm observation_observation_at_subject},
   comparison = @{thm observation_comparison_at_subject}}\<close>
