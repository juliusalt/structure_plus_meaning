...
definition scope_investigation_decision :: "nat \<Rightarrow> nat \<Rightarrow> bool" where
  "scope_investigation_decision f n=scope_subject_operation f (observation_scope_example_argument n)"
...
lemma scope_investigation_decision_code [code]:
  ...
  by (simp only: scope_investigation_decision_def scope_subject_operation_def observation_scope_example_rows_decision observation_scope_encoded
...
lemma scope_subject_decision_at:
  "scope_subject_operation f (observation_scope_example_argument c)=scope_investigation_decision f c"
  by (simp only: scope_investigation_decision_def)

definition scope_candidate_subjects where
  "scope_candidate_subjects=fimage (\<lambda>c. (c,observation_scope_example_argument c)) {|0::nat,1,2,3,4,5,6|}"

definition scope_operation_subjects where
  "scope_operation_subjects=fimage (\<lambda>f. (f,scope_subject_operation f)) {|0::nat,1|}"

definition scope_value_subjects where
  "scope_value_subjects=fimage (\<lambda>w. (w,w=1)) {|0::nat,1|}"

lemma scope_subject_maps_formed:
  "finite_observation_subjects_formed scope_candidate_subjects scope_operation_subjects scope_value_subjects"
  unfolding scope_candidate_subjects_def scope_operation_subjects_def scope_value_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma scope_observations_derived:
  "fset_of_list scope_investigation_observations=
    finite_derived_observations scope_candidate_subjects scope_operation_subjects
      scope_value_subjects (\<lambda>operation subject value. value=operation subject)"
  unfolding scope_candidate_subjects_def scope_operation_subjects_def scope_value_subjects_def
  apply (rule finite_table_derived_from_function_graphs)
  by (simp only: fset_of_list.rep_eq scope_investigation_observations_def indexed_value_rows_member scope_subject_decision_at;
    auto split: if_splits)

theorem scope_observation_at_subject:
  assumes "(c,subject) |\<in>| scope_candidate_subjects"
    "(f,operation) |\<in>| scope_operation_subjects"
    "(w,value) |\<in>| scope_value_subjects"
  shows "(f,c,w)\<in>set scope_investigation_observations \<longleftrightarrow> value=operation subject"
  using finite_derived_observation_at_subject[OF scope_subject_maps_formed assms, where P="\<lambda>operation subject value. value=operation subject"]
  by (simp only: scope_observations_derived[symmetric] fset_of_list.rep_eq)

lemma scope_subject_complete_at:
  "scope_subject_operation 1 (observation_scope_example_argument c)=observation_scope_example_admitted c"
  by (simp add: scope_subject_operation_def observation_scope_encoded observation_scope_example_admitted_def)

theorem scope_comparison_at_subject:
  assumes "(c,subject) |\<in>| scope_candidate_subjects"
    "(d,other) |\<in>| scope_candidate_subjects"
  shows "(c,d)\<in>set scope_investigation_relation \<longleftrightarrow> scope_subject_operation 1 subject=scope_subject_operation 1 other"
  using assms
  by (simp only: scope_candidate_subjects_def finite_function_graph_member
    finsert.rep_eq bot_fset.rep_eq scope_investigation_relation_exact;
    metis scope_subject_complete_at)

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm scope_investigation_def},
   equation = @{thm scope_observations_derived},
   formation = @{thm scope_subject_maps_formed},
   observation = @{thm scope_observation_at_subject},
   comparison = @{thm scope_comparison_at_subject}}\<close>
