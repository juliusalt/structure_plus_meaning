...
proof -
  ...
  have carrier: "rra_carrier (object_structure frame.record_framed)={[]}"
    by (simp add: record_wrapper_def attach_structure_def record_structure_def
      syntax_forest_Nil empty_artifact_def family_ports_empty)
  have rec: "record_at frame.record_framed [] [] []" using frame.record_read by (simp add: family_ports_empty)
  ...
  have same: "assertion_node_syntax=frame.record_framed"
    by (simp add: assertion_node_syntax_def family_ports_empty)
