theory Factor_Schema_Socket_Investigation
  imports Factor_Clause_Specialization_Instances Finite_Observation_Contracts Native_Execution_Refinements
begin
...
definition socket_subject_operation :: "nat \<Rightarrow> local_address option native_schema \<Rightarrow> bool+local_address set" where
  "socket_subject_operation f subject=(if f=0 then Inl (schema_conclusion subject=schema_conclusion native_incidence_schema)
    else if f=1 then Inl (\<forall>v. rel_ran (evaluate_schema_materials v subject)=
      rel_ran (evaluate_schema_materials v native_incidence_schema))
    else Inr (schema_sockets subject))"

definition socket_value_subject :: "nat \<Rightarrow> bool+local_address set" where
  "socket_value_subject w=(if w=0 then Inl True else if w=1 then Inl False
    else if w=18 then Inr {[18]} else Inr {[19]})"

lemma socket_value_subject_at:
  assumes "w\<in>{0,1,18,19}"
  shows "socket_value_subject w=socket_subject_operation f (socket_investigation_schema c)
    \<longleftrightarrow> w=socket_investigation_observation f c"
  using assms by (auto simp: socket_value_subject_def socket_subject_operation_def
    socket_investigation_observation_code socket_investigation_fields socket_investigation_unkeyed_materials)

definition socket_candidate_subjects where
  "socket_candidate_subjects=fimage (\<lambda>c. (c,socket_investigation_schema c)) {|0::nat,1|}"

definition socket_operation_subjects where
  "socket_operation_subjects=fimage (\<lambda>f. (f,socket_subject_operation f)) {|0::nat,1,2|}"

definition socket_value_subjects where
  "socket_value_subjects=fimage (\<lambda>w. (w,socket_value_subject w)) {|0::nat,1,18,19|}"

lemma socket_subject_maps_formed:
  "finite_observation_subjects_formed socket_candidate_subjects socket_operation_subjects socket_value_subjects"
  unfolding socket_candidate_subjects_def socket_operation_subjects_def socket_value_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma socket_observations_derived:
  "fset_of_list schema_sockets_investigation_observations=
    finite_derived_observations socket_candidate_subjects socket_operation_subjects
      socket_value_subjects (\<lambda>operation subject value. value=operation subject)"
  unfolding socket_candidate_subjects_def socket_operation_subjects_def socket_value_subjects_def
  apply (rule finite_table_derived_from_function_graphs)
  using socket_value_subject_at
  by (auto simp: schema_sockets_investigation_observations_def fset_of_list.rep_eq
    socket_value_subject_def socket_subject_operation_def socket_investigation_observation_code
    socket_investigation_fields socket_investigation_unkeyed_materials)

theorem socket_observation_at_subject:
  assumes "(c,subject) |\<in>| socket_candidate_subjects"
    "(f,operation) |\<in>| socket_operation_subjects"
    "(w,value) |\<in>| socket_value_subjects"
  shows "(f,c,w)\<in>set schema_sockets_investigation_observations \<longleftrightarrow> value=operation subject"
  using finite_derived_observation_at_subject[OF socket_subject_maps_formed assms, where P="\<lambda>operation subject value. value=operation subject"]
  by (simp only: socket_observations_derived[symmetric] fset_of_list.rep_eq)

theorem socket_comparison_at_subject:
  assumes "(c,subject) |\<in>| socket_candidate_subjects"
    "(d,other) |\<in>| socket_candidate_subjects"
  shows "(c,d)\<in>set schema_sockets_investigation_relation \<longleftrightarrow> (\<exists>s. other=schema_substitute s subject)"
  using assms by (auto simp: socket_candidate_subjects_def fimage.rep_eq schema_sockets_relation_exact)

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm schema_sockets_investigation_def},
   equation = @{thm socket_observations_derived},
   formation = @{thm socket_subject_maps_formed},
   observation = @{thm socket_observation_at_subject},
   comparison = @{thm socket_comparison_at_subject}}\<close>
