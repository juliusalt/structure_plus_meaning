theory Factor_Substitution_Investigation
  imports Factor_Substitution Factor_Pattern_Determination Finite_Observation_Contracts Native_Execution_Refinements
begin
...
definition pattern_value_subject :: "nat \<Rightarrow> factor_term" where
  "pattern_value_subject w=(if w=0 then Pair_Term (Payload_Term [0]) (Payload_Term [0])
    else if w=1 then Pair_Term (Payload_Term [1]) (Payload_Term [1])
    else Pair_Term (Target_Term (Whole_Artifact empty_artifact)) (Target_Term (Whole_Artifact empty_artifact)))"

lemma pattern_value_subject_at:
  assumes "w\<in>{0,1,2}"
  shows "pattern_value_subject w=evaluate_pattern (pattern_investigation_marker collapsed f) (pattern_investigation_pattern c)
    \<longleftrightarrow> w=pattern_investigation_observation collapsed f c"
proof -
  let ?r="evaluate_pattern (pattern_investigation_marker collapsed f) (pattern_investigation_pattern c)"
  consider (zero) "?r=Pair_Term (Payload_Term [0]) (Payload_Term [0])"
    | (one) "?r=Pair_Term (Payload_Term [1]) (Payload_Term [1])"
    | (target) "?r=Pair_Term (Target_Term (Whole_Artifact empty_artifact)) (Target_Term (Whole_Artifact empty_artifact))"
    using pattern_investigation_output_range[of collapsed f c] by auto
  then show ?thesis
  proof cases
    case zero
    then show ?thesis using assms
      by (auto simp: pattern_value_subject_def pattern_investigation_observation_def)
  next
    case one
    then show ?thesis using assms
      by (auto simp: pattern_value_subject_def pattern_investigation_observation_def)
...
definition pattern_candidate_subjects where
  "pattern_candidate_subjects=fimage (\<lambda>c. (c,pattern_investigation_pattern c)) {|0::nat,1,2,3,4|}"

definition pattern_operation_subjects where
  "pattern_operation_subjects collapsed=fimage (\<lambda>f. (f,pattern_investigation_marker collapsed f)) {|0::nat,1|}"

definition pattern_value_subjects where
  "pattern_value_subjects=fimage (\<lambda>w. (w,pattern_value_subject w)) {|0::nat,1,2|}"

lemma pattern_subject_maps_formed:
  "finite_observation_subjects_formed pattern_candidate_subjects (pattern_operation_subjects collapsed) pattern_value_subjects"
  unfolding pattern_candidate_subjects_def pattern_operation_subjects_def pattern_value_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma pattern_investigation_observation_range:
  "pattern_investigation_observation collapsed f c\<in>{0,1,2}"
  using pattern_investigation_output_range[of collapsed f c]
  by (auto simp: pattern_investigation_observation_def)

lemma pattern_observations_derived:
  "fset_of_list (pattern_investigation_observations collapsed)=
    finite_derived_observations pattern_candidate_subjects (pattern_operation_subjects collapsed)
      pattern_value_subjects (\<lambda>valuation subject value. value=evaluate_pattern valuation subject)"
  unfolding pattern_candidate_subjects_def pattern_operation_subjects_def pattern_value_subjects_def
proof (rule finite_table_derived_from_function_graphs)
  fix f c w
  show "(f,c,w) |\<in>| fset_of_list (pattern_investigation_observations collapsed) \<longleftrightarrow>
    f |\<in>| {|0,1|} \<and> c |\<in>| {|0,1,2,3,4|} \<and> w |\<in>| {|0,1,2|} \<and>
    pattern_value_subject w=evaluate_pattern (pattern_investigation_marker collapsed f) (pattern_investigation_pattern c)"
    using pattern_investigation_observation_range[of collapsed f c]
    by (simp only: fset_of_list.rep_eq pattern_investigation_observations_def
      indexed_value_rows_member finsert.rep_eq bot_fset.rep_eq list.set;
      metis pattern_value_subject_at)
qed

theorem pattern_observation_at_subject:
  assumes "(c,subject) |\<in>| pattern_candidate_subjects"
    "(f,valuation) |\<in>| pattern_operation_subjects collapsed"
    "(w,value) |\<in>| pattern_value_subjects"
  shows "(f,c,w)\<in>set (pattern_investigation_observations collapsed) \<longleftrightarrow>
    value=evaluate_pattern valuation subject"
  using finite_derived_observation_at_subject[OF pattern_subject_maps_formed assms, where P="\<lambda>valuation subject value. value=evaluate_pattern valuation subject"]
  by (simp only: pattern_observations_derived[symmetric] fset_of_list.rep_eq)

theorem pattern_comparison_at_subject:
  assumes "(c,subject) |\<in>| pattern_candidate_subjects"
    "(d,other) |\<in>| pattern_candidate_subjects"
  shows "(c,d)\<in>set pattern_investigation_relation \<longleftrightarrow> subject=other"
  using assms
  by (simp only: pattern_candidate_subjects_def finite_function_graph_member
    finsert.rep_eq bot_fset.rep_eq pattern_investigation_relation_exact;
    metis)

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm pattern_investigation_def},
   equation = @{thm pattern_observations_derived},
   formation = @{thm pattern_subject_maps_formed},
   observation = @{thm pattern_observation_at_subject},
   comparison = @{thm pattern_comparison_at_subject}}\<close>

export_code investigation_inference investigation_basis investigation_repairs investigation_extend pattern_investigation
