...
type_synonym revision_operation = "revision_workload \<Rightarrow> nat list"

definition revision_subject_condition where
  "revision_subject_condition q (method::revision_operation) (problem::revision_workload)=
    (case problem of (C,U,F,T,R) \<Rightarrow>
      revision_quality_condition (set C) (\<lambda>c d. (c,d)\<in>set R) (set U) (set F)
        (finite_table_observations (fset_of_list T)) (set (method problem)) q)"
...
definition revision_candidate_subjects where
  "revision_candidate_subjects=fimage (\<lambda>m. (m,revision_method_selection m)) {|0::nat,1,2,3,4,5|}"

definition revision_condition_subjects where
  "revision_condition_subjects=fimage (\<lambda>f. (f,revision_subject_condition f)) {|0::nat,1,2,3|}"

definition revision_workload_subjects where
  "revision_workload_subjects=fimage (\<lambda>w. (w,revision_workload w)) {|0::nat,1,2,3,4|}"

lemma revision_subject_maps_formed:
  "finite_observation_subjects_formed revision_candidate_subjects revision_condition_subjects revision_workload_subjects"
  unfolding revision_candidate_subjects_def revision_condition_subjects_def revision_workload_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma revision_subject_condition_at:
  "revision_subject_condition q (revision_method_selection m) (revision_workload w)=revision_method_quality m w q"
  using revision_method_quality_exact[of w] by (cases "revision_workload w")
    (auto simp: revision_subject_condition_def split: prod.splits)

lemma revision_observations_derived:
  "fset_of_list revision_investigation_observations=
    finite_derived_observations revision_candidate_subjects revision_condition_subjects
      revision_workload_subjects (\<lambda>condition method problem. condition method problem)"
  unfolding revision_candidate_subjects_def revision_condition_subjects_def revision_workload_subjects_def
  apply (rule finite_table_derived_from_function_graphs)
  by (simp only: fset_of_list.rep_eq revision_investigation_observations_def indexed_predicate_rows_member revision_subject_condition_at; auto)

theorem revision_observation_at_subject:
  assumes "(m,method) |\<in>| revision_candidate_subjects"
    "(f,condition) |\<in>| revision_condition_subjects"
    "(w,problem) |\<in>| revision_workload_subjects"
  shows "(f,m,w)\<in>set revision_investigation_observations \<longleftrightarrow> condition method problem"
  using finite_derived_observation_at_subject[OF revision_subject_maps_formed assms, where P="\<lambda>condition method problem. condition method problem"]
  by (simp only: revision_observations_derived[symmetric] fset_of_list.rep_eq)
...
definition revision_subject_comparison where
  "revision_subject_comparison method other \<longleftrightarrow>
    (\<forall>(f,condition)\<in>fset revision_condition_subjects. \<forall>(w,problem)\<in>fset revision_workload_subjects.
      condition method problem \<longrightarrow> condition other problem)"

theorem revision_comparison_at_subject:
  assumes "(c,method) |\<in>| revision_candidate_subjects"
    "(d,other) |\<in>| revision_candidate_subjects"
  shows "(c,d)\<in>set revision_investigation_relation \<longleftrightarrow> revision_subject_comparison method other"
proof -
  have comparison: "revision_subject_comparison (revision_method_selection c) (revision_method_selection d) \<longleftrightarrow>
    (\<forall>w\<in>set [0,1,2,3,4]. \<forall>q\<in>set [0,1,2,3].
      revision_method_quality c w q \<longrightarrow> revision_method_quality d w q)"
    by (simp only: revision_subject_comparison_def revision_condition_subjects_def
      revision_workload_subjects_def finite_function_graph_all revision_subject_condition_at
      finsert.rep_eq bot_fset.rep_eq list.set; blast)
  have mapped: "c\<in>{0,1,2,3,4,5} \<and> method=revision_method_selection c"
    "d\<in>{0,1,2,3,4,5} \<and> other=revision_method_selection d"
    using assms by (simp_all only: revision_candidate_subjects_def finite_function_graph_member
      finsert.rep_eq bot_fset.rep_eq)
  have indexed: "(c,d)\<in>set revision_investigation_relation \<longleftrightarrow>
    c\<in>{0,1,2,3,4,5} \<and> d\<in>{0,1,2,3,4,5} \<and>
    (\<forall>w\<in>set [0,1,2,3,4]. \<forall>q\<in>set [0,1,2,3].
      revision_method_quality c w q \<longrightarrow> revision_method_quality d w q)"
    by (simp only: revision_investigation_relation_def indexed_relation_rows_member list.set)
  show ?thesis using mapped indexed comparison by blast
qed

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm revision_investigation_def},
   equation = @{thm revision_observations_derived},
   formation = @{thm revision_subject_maps_formed},
