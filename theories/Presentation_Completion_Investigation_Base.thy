theory Presentation_Completion_Investigation_Base
  imports Finite_Observation_Contracts Presentation_Completion
begin

section \<open>Joint feasibility distinguishes separately completable tests\<close>

definition completion_investigation_test :: "nat \<Rightarrow> bool \<Rightarrow> bool" where
  "completion_investigation_test c=(if c=0 then id else Not)"

definition completion_subject_operation :: "nat \<Rightarrow> (bool\<Rightarrow>bool)\<times>(bool\<Rightarrow>bool) \<Rightarrow> bool" where
  "completion_subject_operation f subject=(case subject of (left,right) \<Rightarrow>
    (if f=0 then saturate_observation (\<lambda>_::unit. \<lambda>_::bool. True) left False
     else if f=1 then saturate_observation (\<lambda>_::unit. \<lambda>_::bool. True) right False
     else saturate_observation (\<lambda>_::unit. \<lambda>_::bool. True) (\<lambda>p. left p \<and> right p) False))"

definition completion_investigation_observe :: "nat \<Rightarrow> nat \<Rightarrow> bool" where
  "completion_investigation_observe f c=completion_subject_operation f (id,completion_investigation_test c)"

lemma completion_investigation_observe_exact:
  "completion_investigation_observe f c \<longleftrightarrow> f=0 \<or> f=1 \<or> c=0"
  by (auto simp: completion_investigation_observe_def completion_subject_operation_def completion_investigation_test_def
    saturate_observation_def presentation_transport_def)

definition completion_investigation_observations :: "(nat\<times>nat\<times>nat) list" where
  "completion_investigation_observations=concat (map (\<lambda>f.
    map (\<lambda>c. (f,c,0)) (filter (completion_investigation_observe f) [0,1])) [0,1,2])"

definition completion_investigation_relation :: "(nat\<times>nat) list" where
  "completion_investigation_relation=filter (\<lambda>(c,d).
    completion_investigation_observe 2 c \<longrightarrow> completion_investigation_observe 2 d)
      (investigation_pairs [0,1])"

lemma completion_investigation_observations_exact:
  "finite_table_observations (fset_of_list completion_investigation_observations) f c=
    (if f\<in>{0,1,2} \<and> c\<in>{0,1} \<and> completion_investigation_observe f c then {0} else {})"
  by (auto simp: finite_table_observations_def completion_investigation_observations_def completion_investigation_observe_exact)

lemma completion_investigation_relation_exact:
  "(c,d)\<in>set completion_investigation_relation \<longleftrightarrow>
    c\<in>{0,1} \<and> d\<in>{0,1} \<and>
      (completion_investigation_observe 2 c \<longrightarrow> completion_investigation_observe 2 d)"
  by (auto simp: completion_investigation_relation_def investigation_pairs_exact)

definition completion_investigation where
  "completion_investigation selected=investigation_basis [0,1] [0,1,2] selected
    completion_investigation_observations completion_investigation_relation"

theorem component_observations_miss_joint_failure:
  "set (fst (snd (completion_investigation [0,1])))={(0,1)}"
  by (simp add: completion_investigation_def investigation_basis_def Let_def
    investigation_select_def investigation_pairs_def completion_investigation_observations_def
    completion_investigation_relation_def completion_investigation_observe_exact
    finite_basis_residual_def finite_candidate_profile_def)

theorem the_joint_observation_is_an_adequate_basis:
  "fst (completion_investigation [2]) \<and> fst (snd (completion_investigation [2]))=[]"
  by (simp add: completion_investigation_def investigation_basis_def Let_def
    investigation_select_def investigation_pairs_def completion_investigation_observations_def
    completion_investigation_relation_def completion_investigation_observe_exact
    finite_basis_residual_def finite_candidate_profile_def finite_basis_evaluation_def
    finite_observation_table_formed_def)

theorem no_selected_observation_misses_joint_failure:
  "set (fst (snd (completion_investigation [])))={(0,1)}"
  by (simp add: completion_investigation_def investigation_basis_def Let_def
    investigation_select_def investigation_pairs_def completion_investigation_observations_def
    completion_investigation_relation_def completion_investigation_observe_exact
    finite_basis_residual_def finite_candidate_profile_def)

lemma completion_subject_decision_at:
  "completion_subject_operation f (id,completion_investigation_test c)=completion_investigation_observe f c"
  by (simp only: completion_investigation_observe_def)

definition completion_candidate_subjects :: "(nat\<times>((bool\<Rightarrow>bool)\<times>(bool\<Rightarrow>bool))) fset" where
  "completion_candidate_subjects=fimage (\<lambda>c. (c,(id,completion_investigation_test c))) {|0::nat,1|}"

definition completion_operation_subjects where
  "completion_operation_subjects=fimage (\<lambda>f. (f,completion_subject_operation f)) {|0::nat,1,2|}"

definition completion_value_subjects where
  "completion_value_subjects=fimage (\<lambda>w. (w,())) {|0::nat|}"

lemma completion_subject_maps_formed:
  "finite_observation_subjects_formed completion_candidate_subjects completion_operation_subjects completion_value_subjects"
  unfolding completion_candidate_subjects_def completion_operation_subjects_def completion_value_subjects_def
  by (rule finite_observation_function_graphs_formed)

lemma completion_observations_derived:
  "fset_of_list completion_investigation_observations=
    finite_derived_observations completion_candidate_subjects completion_operation_subjects
      completion_value_subjects (\<lambda>operation subject _. operation subject)"
  unfolding completion_candidate_subjects_def completion_operation_subjects_def completion_value_subjects_def
  apply (rule finite_table_derived_from_function_graphs)
  by (simp only: fset_of_list.rep_eq completion_investigation_observations_def
    indexed_satisfied_rows_member completion_subject_decision_at; auto)

theorem completion_observation_at_subject:
  assumes "(c,subject) |\<in>| completion_candidate_subjects"
    "(f,operation) |\<in>| completion_operation_subjects"
    "(w,value) |\<in>| completion_value_subjects"
  shows "(f,c,w)\<in>set completion_investigation_observations \<longleftrightarrow> operation subject"
  using finite_derived_observation_at_subject[OF completion_subject_maps_formed assms, where P="\<lambda>operation subject _. operation subject"]
  by (simp only: completion_observations_derived[symmetric] fset_of_list.rep_eq)

theorem completion_comparison_at_subject:
  assumes "(c,subject) |\<in>| completion_candidate_subjects"
    "(d,other) |\<in>| completion_candidate_subjects"
  shows "(c,d)\<in>set completion_investigation_relation \<longleftrightarrow> (completion_subject_operation 2 subject \<longrightarrow> completion_subject_operation 2 other)"
  using assms by (auto simp: completion_candidate_subjects_def fimage.rep_eq completion_investigation_relation_exact completion_subject_decision_at)

setup \<open>Finite_Observation_Contracts.register
  {definition = @{thm completion_investigation_def},
   equation = @{thm completion_observations_derived},
   formation = @{thm completion_subject_maps_formed},
   observation = @{thm completion_observation_at_subject},
   comparison = @{thm completion_comparison_at_subject}}\<close>

text \<open>
  The two candidates pair the identity test with identity or negation on the
  complete Boolean presentation fibre of one unit subject. Observations are
  computed by the existing existential completion operation. Both component
  tests can succeed separately. Only the first pair can use a common witness.

  The intended comparison preserves joint feasibility. Component observations
  identify a feasible pair with an infeasible pair, returning the exact failed
  direction. The joint observation suffices by itself; adding the two weaker
  facets supplies no further distinction for this comparison. An empty basis
  fails. The numeric coordinates present this finite case and introduce no
  new semantic kind, global method enumeration, or native permission proof.
\<close>

end
