...
lemma environment_position_anchor:
  assumes ef: "environment_formed E" and position: "d\<in>environment_positions E"
  shows "\<exists>R. artifact_at E (fst d) R \<and> anchor_formed (R,snd d)"
proof -
  obtain u a where shape: "d=(u,a)" by (cases d) auto
  obtain R where source: "artifact_at E u R" "a\<in>rra_carrier (object_structure R)"
    using position shape by auto
  have rf: "exact_formed R" using ef source(1) unfolding environment_formed_def by blast
  show ?thesis using source rf shape by (auto simp: anchor_formed_def)
qed

lemma environment_position_address:
  assumes "environment_formed E" "d\<in>environment_positions E"
  shows "octets_formed (snd d)"
  using environment_position_anchor[OF assms]
  by (auto simp: anchor_formed_def exact_formed_def)

lemma environment_positions_included:
  assumes "environment_included E F"
  shows "environment_positions E\<subseteq>environment_positions F"
  using assms by (auto simp: environment_positions_def environment_included_def)
