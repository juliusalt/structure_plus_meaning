...
lemma equal_footprint_data_restrictions:
  assumes eq: "footprint_of obj A = footprint_of other A"
    and inside: "B \<subseteq> footprint_interior (footprint_of obj A)"
  shows "restrict_basis B (object_data obj) = restrict_basis B (object_data other)"
proof -
  have ieq: "footprint_interior (footprint_of obj A) = footprint_interior (footprint_of other A)"
    and deq: "footprint_data (footprint_of obj A) = footprint_data (footprint_of other A)"
    using eq by auto
  have left: "B \<inter> (rra_carrier (object_structure obj) \<inter> A) = B"
    and right: "B \<inter> (rra_carrier (object_structure other) \<inter> A) = B"
    using inside ieq by (auto simp: footprint_of_def Let_def)
  have "restrict_basis B (footprint_data (footprint_of obj A)) =
    restrict_basis B (footprint_data (footprint_of other A))"
    using deq by simp
  then show ?thesis
    by (simp add: footprint_of_def Let_def restriction_composes left right)
qed
