...
lemma literal_interior_finite [simp]: "finite (literal_interior t)"
...
lemma literal_citation_mapped_interpretation:
  assumes "target_formed t" "external_slot_values E u (f [4]) = {target_artifact t}"
  shows "interpret_citation E u (map_citation_positions f (literal_citation t)) t"
  using assms
  by (cases t)
     (auto simp: external_interpretation_by_slot split: prod.splits
       simp del: interpret_citation.simps)
