...
fun syntax_forest :: "exact_artifact list \<Rightarrow> exact_artifact" where
  "syntax_forest [] = empty_artifact"
| "syntax_forest (R#Rs) = syntax_union R (syntax_forest Rs)"
...
lemma syntax_branch_successor: "syntax_branch (Suc n) = Cons 3 \<circ> syntax_branch n" by (rule ext) simp
...
lemma syntax_forest_no_counts [simp]: "bag_count (object_data (syntax_forest Rs)) = (\<lambda>_. 0)"
  by (cases Rs) (simp_all add: empty_artifact_def empty_basis_def)

lemma syntax_forest_formed:
  assumes "\<forall>R\<in>set Rs. exact_formed R"
  shows "exact_formed (syntax_forest Rs)"
  using assms by (induction Rs) (auto intro: syntax_union_formed)

lemma syntax_forest_empty_absent [simp]:
  "[] \<notin> rra_carrier (object_structure (syntax_forest Rs))"
  by (cases Rs) (auto simp: empty_artifact_def syntax_union_def)
...
lemma syntax_forest_prefix_absent [simp]:
  ...
  using assms by (cases Rs) (auto simp: empty_artifact_def syntax_union_def)
...
lemma syntax_forest_child_inside:
  ...
  using index atom
proof (induction Rs arbitrary: i)
  case Nil
...
next
  ...
  proof (cases i)
    case 0
    show ?thesis using Cons.prems(2) by (auto simp: 0 syntax_union_def)
  next
    case (Suc n)
    have small: "n < length Rs" and inside: "a \<in> rra_carrier (object_structure (Rs!n))" using Cons.prems Suc by simp_all
    have tail: "syntax_branch n a \<in> rra_carrier (object_structure (syntax_forest Rs))" by (rule Cons.IH[OF small inside])
    show ?thesis using tail by (auto simp: Suc syntax_union_def)
  qed
qed
...
lemma syntax_forest_atom_origin:
  ...
  using assms
proof (induction Rs arbitrary: a)
  case Nil
  then show ?case by (simp add: empty_artifact_def)
next
  case (Cons R Rs)
  have alternatives: "a \<in> Cons 2 ` rra_carrier (object_structure R) \<or>
    a \<in> Cons 3 ` rra_carrier (object_structure (syntax_forest Rs))"
    using Cons.prems by (simp add: syntax_union_def)
  then show ?case
  proof
    assume left: "a \<in> Cons 2 ` rra_carrier (object_structure R)"
    obtain b where old: "b \<in> rra_carrier (object_structure R)" "a=2#b" using left by blast
    show ?thesis by (rule exI[of _ 0]) (use old in auto)
  next
    assume right: "a \<in> Cons 3 ` rra_carrier (object_structure (syntax_forest Rs))"
    obtain c where old: "c \<in> rra_carrier (object_structure (syntax_forest Rs))" "a=3#c" using right by blast
    obtain i b where origin: "i < length Rs" "b \<in> rra_carrier (object_structure (Rs!i))" "c=syntax_branch i b"
      using Cons.IH[OF old(1)] by blast
    show ?thesis by (rule exI[of _ "Suc i"]) (use old origin in auto)
  qed
qed
...
next
  ...
  proof (cases i)
    case 0
    have zero: "bag_count (object_data R) = (\<lambda>_. 0)" using Cons.prems(2) by simp
    show ?thesis using syntax_union_reads_left[OF zero, of "syntax_forest Rs"]
      by (simp only: 0 syntax_branch_zero syntax_forest.simps nth_Cons_0)
  next
    case (Suc n)
    have tail_formed: "\<forall>T\<in>set Rs. exact_formed T"
      and tail_counts: "\<forall>T\<in>set Rs. bag_count (object_data T) = (\<lambda>_. 0)"
      and small: "n < length Rs" using Cons.prems Suc by simp_all
    have old: "object_reads_agree (push_object (syntax_branch n) (Rs!n)) (syntax_forest Rs)
      (syntax_branch n ` rra_carrier (object_structure (Rs!n)))"
      by (rule Cons.IH[OF tail_formed tail_counts small])
    have child: "object_formed (Rs!n)" using tail_formed nth_mem[OF small] by (auto simp: exact_formed_def)
    have inj: "inj_on (syntax_branch n) (rra_carrier (object_structure (Rs!n)))"
      using syntax_branch_injective[of n] by (auto simp: inj_on_def)
    have copied: "object_formed (push_object (syntax_branch n) (Rs!n))"
      using object_push_isomorphism[OF child inj] by (simp add: object_isomorphism_def)
    have tf: "object_formed (syntax_forest Rs)"
      using syntax_forest_formed[OF tail_formed] by (simp add: exact_formed_def)
    have injective: "inj (Cons 3 :: local_address \<Rightarrow> local_address)" by (simp add: inj_def)
    have pushed: "object_reads_agree (push_object (Cons 3) (push_object (syntax_branch n) (Rs!n)))
      (push_object (Cons 3) (syntax_forest Rs))
      (Cons 3 ` (syntax_branch n ` rra_carrier (object_structure (Rs!n))))"
      by (rule object_reads_agree_push[OF copied tf injective old])
    have right: "object_reads_agree (push_object (Cons 3) (syntax_forest Rs))
      (syntax_union R (syntax_forest Rs))
      (rra_carrier (object_structure (push_object (Cons 3) (syntax_forest Rs))))"
      using syntax_union_reads_right[OF syntax_forest_no_counts, of Rs R]
      by (simp add: push_object_def push_structure_def)
    have extended: "object_reads_agree (push_object (Cons 3) (push_object (syntax_branch n) (Rs!n)))
      (syntax_union R (syntax_forest Rs))
      (Cons 3 ` (syntax_branch n ` rra_carrier (object_structure (Rs!n))))"
      by (rule object_reads_agree_extend[OF pushed right])
    show ?thesis using extended by (simp only: Suc syntax_forest.simps nth_Cons_Suc
        syntax_branch_successor push_object_composes[OF child] image_image comp_def)
  qed
qed

text \<open>
  Complete scopes are copied with every occurrence, including each private
  binder. The two copies are disjoint and the union adds no occurrence or
