"""Execute native constructor conditions, complete subjects and computed revisions."""
...
def program(engine, inputs):
    code = check_native_admission.serialization(engine, 'Native_Admission_Investigation_Execution')
...
fun assess w m = let val a = N.native_admission_assess m w in
  "{\"method\":" ^ jnat m ^ ",\"assessment\":" ^ jassessment a ^
    ",\"qualities\":" ^ jlist jquality (map (fn f => (f,N.native_admission_optional_inspect a f)) facets) ^ "}" end;
fun emitSubject w = print ("ADMISSION_SUBJECT " ^ jnat w ^ " " ^
  jreport (N.native_admission_report w) ^ "\n");
fun emitAssessment w = print ("ADMISSION_ASSESSMENT " ^ jnat w ^ " " ^
...
val () = print ("ADMISSION_SCOPE " ^ jlist jnat N.native_admission_indices ^ "\n");
val () = List.app emitSubject scope;
val () = List.app emitAssessment scope;
val ((rows,(relation,(selected,adequate))),cycles) = N.native_admission_investigation_report scope selections;
val () = print ("ADMISSION_COMPARISON {\"scope\":" ^ jlist jnat scope ^
...
val () = List.app (fn c => print ("ADMISSION_INVESTIGATION " ^ jcycle c ^ "\n")) cycles;
'''
    return code
...
def main():
    ...
    contract = observation_contracts.read_contract(Path(contracts[0]['path']),
        'Factor_Native_Admission_Investigation', 'native_admission_investigation')

    def assess(inputs, raw):
        path = args.output / 'results.log'
        assert path.read_text() == raw
        reports = list(machine_reports.reports(path))
        assert reports and reports[0]['tag'] == 'ADMISSION_SCOPE' and reports[0]['indices'] == []
        ...
        assert all(r['tag'] == 'ADMISSION_SUBJECT' and r['indices'] == [w]
                   for w, r in zip(requested, reports[1:1 + size]))
        assert all(r['tag'] == 'ADMISSION_ASSESSMENT' and r['indices'] == [w]
                   for w, r in zip(requested, reports[1 + size:1 + 2 * size]))
        compared = reports[1 + 2 * size]
        assert compared['tag'] == 'ADMISSION_COMPARISON' and compared['indices'] == []
        assert compared['value']['scope'] == requested
        assert all(r['tag'] == 'ADMISSION_INVESTIGATION' and r['indices'] == []
    ...
    receipt = proved_code.checked_execution(
        args.proof, args.poly, args.output, required_theories=['Native_Admission_Investigation_Execution'],
        ...
        question='Which actual native constructor operations satisfy the original goal, preserve the '
                 'complete source environment, and refuse unsupported source requests?',
        boundary='The exact observation equation relates actual candidate functions, original positive '
