"""Execute the shared development cycle on preserved and supplied native inputs."""
...
def program(engine, inputs):
    code = check_native_certificates.preamble(engine, inputs, 'Native_Certificate_Input_Execution', 'native_certificate_input_indices')
    code += native_certificate_coverage_json.PRELUDE + native_certificate_input_json.PRELUDE
    code += r'''fun emitInputSubject (w,(x,(original,cells))) = print ("CERTIFICATE_SUBJECT " ^ jnat w ^
  " {\"problem\":" ^ jinputReport x ^ ",\"original\":" ^ jderivation original ^
  ",\"candidates\":" ^ jlist jcandidate cells ^ "}\n");

val () = print ("CERTIFICATE_SCOPE " ^ jlist jnat N.native_certificate_input_indices ^ "\n");
val (table,(criticism,(comparison,(cycle,admissions)))) =
  N.native_certificate_input_packet scope (hd selections);
val () = List.app emitInputSubject table;
...
def main():
    ...
    contract = observation_contracts.read_contract(Path(contracts[0]['path']),
        'Factor_Native_Certificate_Input_Development', 'native_certificate_input_investigation',
    ...
    receipt = proved_code.checked_execution(
        args.proof, args.poly, args.output, required_theories=['Native_Certificate_Input_Execution'],
        inputs={'candidates': contract['candidate_indices'], 'facets': contract['facet_indices'],
                'cases': args.cases, 'selections': [list(range(7))]},
        input_paths=[Path(__file__), Path(shared.__file__), Path(native_certificate_coverage_json.__file__), Path(native_certificate_input_json.__file__),
