"""Execute actual native node constructors and their complete condition investigation."""
...
def program(engine, inputs):
    ...
    code += 'structure N = Native_Node_Investigation_Execution;\n'
    ...
    scope = ('N.native_node_indices' if inputs['cases'] is None else
...
fun jtargetRow (s,n) = "[" ^ jsite s ^ "," ^ jsite n ^ "]";
fun jproblem (e,(n,d)) = "{\"environment\":" ^ jenv e ^ ",\"node\":" ^ jnode n ^
  ",\"targets\":" ^ jf jtargetRow d ^ "}";
fun jresult NONE = "null"
  | jresult (SOME (f,u)) = "{\"environment\":" ^ jenv f ^ ",\"use\":" ^ juse u ^ "}";
fun jreading ((n,d),(i,k)) = "{\"node\":" ^ jnode n ^ ",\"targets\":" ^ jf jtargetRow d ^
  ",\"interior\":" ^ jf jaddress i ^ ",\"slots\":" ^ jf jaddress k ^ "}";
fun jbody NONE = "null"
  | jbody (SOME (readings,(formed,(node,(targets,(source,fresh)))))) =
      "{\"readings\":" ^ jf jreading readings ^ ",\"formed\":" ^ Bool.toString formed ^
      ",\"node_retained\":" ^ Bool.toString node ^ ",\"targets_retained\":" ^ Bool.toString targets ^
      ",\"source_retained\":" ^ Bool.toString source ^ ",\"fresh\":" ^ Bool.toString fresh ^ "}";
...
fun jassessed (m,(result,a)) = "{\"method\":" ^ jnat m ^ ",\"assessment\":" ^ jassessment a ^
  ",\"qualities\":" ^ jlist jquality (map (fn f => (f,N.native_node_inspect a f)) facets) ^ "}";
fun emitSubject (w,(x,cells)) = print ("NODE_SUBJECT " ^ jnat w ^ " {\"problem\":" ^ jproblem x ^
  ",\"candidates\":" ^ jlist jcandidate cells ^ "}\n");
fun emitAssessment (w,(x,cells)) = print ("NODE_ASSESSMENT " ^ jnat w ^ " " ^ jlist jassessed cells ^ "\n");
...
val () = print ("NODE_SCOPE " ^ jlist jnat N.native_node_indices ^ "\n");
val (table,(comparison,cycles)) = N.native_node_report_packet scope selections;
val () = List.app emitSubject table;
val () = List.app emitAssessment table;
val (rows,(relation,(selected,adequate))) = comparison;
val () = print ("NODE_COMPARISON {\"scope\":" ^ jlist jnat scope ^
...
val () = List.app (fn c => print ("NODE_INVESTIGATION " ^ jcycle c ^ "\n")) cycles;
'''
    return code
...
def main():
    ...
    contract = observation_contracts.read_contract(Path(contracts[0]['path']),
        'Factor_Native_Node_Investigation', 'native_node_investigation')

    def assess(inputs, raw):
        path = args.output / 'results.log'
        assert path.read_text() == raw
        reports = list(machine_reports.reports(path))
        assert reports and reports[0]['tag'] == 'NODE_SCOPE' and reports[0]['indices'] == []
        ...
        assert all(r['tag'] == 'NODE_SUBJECT' and r['indices'] == [w]
                   for w, r in zip(requested, reports[1:1 + size]))
        assert all(r['tag'] == 'NODE_ASSESSMENT' and r['indices'] == [w]
                   for w, r in zip(requested, reports[1 + size:1 + 2 * size]))
        assert reports[1 + 2 * size]['tag'] == 'NODE_COMPARISON'
        assert reports[1 + 2 * size]['value']['scope'] == requested
        assert all(r['tag'] == 'NODE_INVESTIGATION' and r['indices'] == []
        ...
        return {'complete_scope': scope, 'executed_scope': requested,
                ...
                'boundary': 'The complete actual source, original metadata, returned environments, '
                            'native readings, condition assessments, comparisons and revisions are retained. '
                            'The proved native packet constructs all semantic observations and decisions.'}

    receipt = proved_code.checked_execution(
        args.proof, args.poly, args.output, required_theories=['Native_Node_Investigation_Execution'],
        inputs={'candidates': contract['candidate_indices'], 'facets': contract['facet_indices'],
                'cases': args.cases,
                'selections': [[], [0, 3, 4, 5], [0, 1, 2, 3, 4, 5], [5, 3, 0, 1, 2, 4]]},
        ...
        question='Which actual local node constructors recover the original complete metadata in a fresh '
                 'native node while preserving every original artifact and binding and refusing unavailable inputs?',
        boundary='The observation equation connects actual methods, complete source and metadata inputs, and '
                 'six independent installation conditions. Complete native readings supply both metadata fields. '
                 'The separate universal compiler and installation contracts remain required. This finite '
                 'comparison does not validate an inference, construct or replay a whole proof graph, check '
                 'native mathematical proofs, enforce the complete development cycle, establish the whole cost '
                 'account or authorize genesis.')
