#!/usr/bin/env python3
"""Execute learned reasoning and independently check its complete finite reports.

The receipt distinguishes structural generation controls from the native
counter and installed-input examples. Arbitrary control libraries have no
asserted semantic soundness contract. Their finite matching is checked here.
"""
from __future__ import annotations

import argparse
import copy
import hashlib
import itertools
import json
from pathlib import Path
import subprocess
import sys
import traceback
import uuid

import investigate
import proved_code
from evidence_io import write_json
from reasoning_known_calls import KnownCallContract

ROOT = Path(__file__).resolve().parents[1]


def digest(path):
    return hashlib.sha256(Path(path).read_bytes()).hexdigest()


def clone_json(value):
    """Copy a public JSON value without deepcopy's additional recursive frames."""
    return json.loads(json.dumps(value, separators=(",", ":")))


def check_expected_report(item, actual):
    """Check declared outcomes at exact goal occurrences, including the input gate."""
    expected = item.get("expected_report")
    if expected is None:
        return
    assert actual["input_formed"] == expected["input_formed"], (item["name"], "expected input gate")
    residual = set(map(freeze, actual["residual"]))
    for key, settled in [("settled_goals", True), ("residual_goals", False)]:
        for index in expected.get(key, []):
            assert type(index) is int and 0 <= index < len(item["goals"])
            assert actual["input_formed"], (item["name"], "settlement requires a formed input")
            assert (freeze(item["goals"][index]) not in residual) == settled, (item["name"], key, index)


def loaded_tool_sources():
    """Track imported local helpers as well as the entry-point checker."""
    directory = Path(__file__).resolve().parent
    paths = set()
    for module in tuple(sys.modules.values()):
        name = getattr(module, "__file__", None)
        if name:
            path = Path(name).resolve()
            if path.is_relative_to(directory) and path.is_file():
                paths.add(path)
    return sorted(paths)


def freeze(value):
    if isinstance(value, dict):
        return tuple((k, freeze(v)) for k, v in sorted(value.items()))
    if isinstance(value, list):
        return tuple(map(freeze, value))
    return value


def payload(*values):
    return {"payload": list(values)}


def pair(a, b):
    return {"pair": [a, b]}


def variable(n):
    return {"var": n}


EMPTY = {"empty_artifact": True}


def counter(n):
    result = payload()
    for _ in range(n):
        result = pair(payload(), result)
    return result


def direct_pair(a, b, n):
    goal = pair(payload(1), pair(pair(payload(0), a), pair(payload(0), b)))
    instruction = pair(n, pair(a, pair(b, payload())))
    return pair(pair(goal, n), pair(n, pair(pair(payload(), n), pair(instruction, payload()))))


def specialization_context(a, d, c, b, q):
    return pair(pair(a, pair(d, c)), pair(b, q))


def schema(head, premises=(), materials=()):
    return {"head": head, "premises": list(premises), "materials": list(materials)}


def library(entry, rule, enumeration=None):
    return [{"entry": entry, "schema": rule,
             "enumeration": copy.deepcopy(rule["premises"] if enumeration is None else enumeration)}]


def case(name, lib, frontier, known, goals, operation=None):
    return {"name": name, "library": lib, "frontier": frontier, "known": known,
            "goals": goals, "operation": operation}


def cases():
    rule = schema(direct_pair(variable(0), variable(1), variable(2)),
                  [[i, 340, variable(i)] for i in range(3)])
    lib = library(341, rule)
    frontier = [[340, counter(i)] for i in range(3)]
    goal = [0, 341, direct_pair(counter(0), counter(1), counter(2))]
    result = [case("native-stage-" + str(stage), [] if stage == 0 else lib,
                   frontier, frontier[:2] if stage == 1 else frontier, [goal], ["stage", stage])
              for stage in range(3)]
    false_frontier = frontier[:2] + [[340, payload(0)]]
    false_goal = [0, 341, direct_pair(counter(0), counter(1), payload(0))]
    result += [case("native-false-" + str(trust).lower(), lib, false_frontier,
                    false_frontier if trust else frontier[:2], [false_goal], ["false", trust])
               for trust in [False, True]]
    fresh = [[340, counter(i)] for i in [3, 4, 5]]
    result.append(case("future-counter-values", lib, fresh, fresh,
                       [[17, 341, direct_pair(counter(5), counter(3), counter(4))]]))
    result.append(case("reordered-repeated-frontier", lib, frontier[::-1] + frontier,
                       frontier, [goal, [19, goal[1], goal[2]]]))
    x, y = payload(7), payload(8)
    joined = schema(variable(0), [[0, 10, variable(0)], [1, 11, variable(0)]])
    result += [case("shared-variable-" + str(agree).lower(), library(9, joined),
                    [[10, x], [11, x if agree else y]], [[10, x], [11, x if agree else y]], [[0, 9, x]])
               for agree in [False, True]]
    repeated = schema(variable(0), [[0, 10, variable(0)], [1, 10, variable(0)]])
    result.append(case("separate-premise-occurrences", library(9, repeated), [[10, x]], [], [[0, 9, x]]))
    result.append(case("premise-order", library(9, repeated, repeated["premises"][::-1]),
                       [[10, x]], [[10, x]], [[0, 9, x]]))
    result.append(case("repeated-enumeration", library(9, repeated, repeated["premises"] * 2),
                       [[10, x]], [[10, x]], [[0, 9, x]]))
    result.append(case("missing-premise-enumeration", library(9, repeated, repeated["premises"][:1]),
                       [[10, x]], [[10, x]], [[0, 9, x]]))
    result.append(case("extra-premise-enumeration", library(9, repeated, repeated["premises"] + [[2, 10, variable(0)]]),
                       [[10, x]], [[10, x]], [[0, 9, x]]))
    result.append(case("premise-only-variable", library(9, schema(payload(), [[0, 10, variable(8)]])),
                       [[10, x]], [[10, x]], [[0, 9, payload()]]))
    result.append(case("head-variable-without-premise", library(9, schema(variable(8))), [], [], [[0, 9, x]]))
    material = [variable(8), EMPTY, EMPTY, EMPTY, EMPTY]
    result.append(case("material-only-variable", library(9, schema(payload(), [], [[2, material]])),
                       [[10, EMPTY]], [[10, EMPTY]], [[0, 9, payload()]]))
    result.append(case("complete-empty-material", library(9, schema(payload(), [[0, 10, variable(8)]], [[2, material]])),
                       [[10, EMPTY]], [[10, EMPTY]], [[0, 9, payload()]]))
    for field in range(5):
        changed = copy.deepcopy(material)
        changed[field] = payload()
        result.append(case("false-material-field-" + str(field),
                           library(9, schema(payload(), [[0, 10, variable(8)]], [[2, changed]])),
                           [[10, EMPTY]], [[10, EMPTY]], [[0, 9, payload()]]))
    result.append(case("conflicting-premise-socket", library(9,
                       schema(variable(0), [[0, 10, variable(0)], [0, 11, variable(0)]])),
                       [[10, x], [11, x]], [[10, x], [11, x]], [[0, 9, x]]))
    result.append(case("ordinary-material-socket-collision", library(9,
                       schema(payload(), [[0, 10, variable(8)]], [[0, material]])),
                       [[10, EMPTY]], [[10, EMPTY]], [[0, 9, payload()]]))
    base = case("base", library(9, schema(variable(0), [[0, 10, variable(0)]])),
                [[10, x]], [[10, x]], [[0, 9, x]])
    for field in ["library", "frontier", "known", "goals"]:
        changed = copy.deepcopy(base)
        changed["name"] = "malformed-unused-" + field
        changed[field] += (library(99, schema(payload(256))) if field == "library" else
                           [[8, 99, payload(256)]] if field == "goals" else [[99, payload(256)]])
        result.append(changed)
    changed = copy.deepcopy(base)
    changed.update(name="conflicting-goal-socket", goals=[[0, 9, x], [0, 9, y]])
    result.append(changed)
    body = specialization_context(*(variable(i) for i in range(5)))
    report_rule = schema(pair(body, variable(5)),
                         [[0, 294, body], [1, 126, pair(variable(4), variable(5))]])
    report_lib = library(342, report_rule)
    a, d, c, b, q, other_q, report, other_report = [payload(i) for i in range(40, 48)]
    context = specialization_context(a, d, c, b, q)
    other_context = specialization_context(a, d, c, b, other_q)
    report_frontier = [[294, context], [126, pair(q, report)]]
    report_goal = [0, 342, pair(context, report)]
    controls = [case("report-without-rule", [], report_frontier, [], [report_goal]),
        case("report-matching-target", report_lib, report_frontier, [], [report_goal], ("report", None)),
        case("report-different-target", report_lib,
             [[294, context], [126, pair(other_q, report)]], [], [report_goal], ("report", None)),
        case("report-two-targets", report_lib,
             report_frontier + [[294, other_context], [126, pair(other_q, other_report)]], [],
             [report_goal, [1, 342, pair(other_context, other_report)]], ("report", None)),
        case("report-two-possible-reports", report_lib,
             report_frontier + [[126, pair(q, other_report)]], [],
             [report_goal, [1, 342, pair(context, other_report)]], ("report", None)),
        case("report-reordered-repeated-frontier", report_lib,
             list(reversed(report_frontier)) + report_frontier, [], [report_goal], ("report", None)),
        case("report-unused-malformed-source", report_lib,
             report_frontier + [[126, payload(256)]], [], [report_goal], ("report", None)),
        case("report-distinct-goal-occurrences", report_lib, report_frontier, [],
             [report_goal, [17, 342, pair(context, report)]], ("report", None))]
    for control in controls:
        control["semantic_boundary"] = (
            "Structural construction control. Both native source premises remain unproved; the known set is empty.")
    result.extend(controls)
    result.extend(binding_cases())
    return result


def sequence(terms):
    result = payload()
    for term in reversed(terms):
        result = pair(term, result)
    return result


def sequence_items(term):
    result = []
    while term != payload():
        if set(term) != {"pair"} or len(term["pair"]) != 2:
            raise ValueError("Not a complete sequence")
        first, term = term["pair"]
        result.append(first)
    return result


def binding_views(owner, rows):
    """Independent row equations, including formation of unreturned fields."""
    if not formed(owner) or not all(map(formed, rows)):
        return None
    fields = []
    for row in rows:
        if set(row) != {"pair"} or len(row["pair"]) != 2:
            return None
        key, values = row["pair"]
        if set(key) != {"pair"} or set(values) != {"pair"}:
            return None
        if len(key["pair"]) != 2 or len(values["pair"]) != 2 or key["pair"][0] != owner:
            return None
        fields.append((key["pair"][1], *values["pair"]))
    return [[pair(key, x) for key, x, y in fields], [pair(key, y) for key, x, y in fields]]


def binding_frontier(owner, rows):
    outputs = binding_views(owner, rows)
    return [] if outputs is None else [[d, pair(owner, pair(sequence(rows), sequence(values)))]
                                      for d, values in zip([345, 346], outputs)]


def binding_call_true(entry, term):
    try:
        owner, body = term["pair"]
        rows, output = body["pair"]
        if entry in {343, 344}:
            views = binding_views(owner, [rows])
            return views is not None and output == views[entry - 343][0]
        views = binding_views(owner, sequence_items(rows))
        if views is None:
            return False
        if entry in {345, 346}:
            return sequence_items(output) == views[entry - 345]
        if entry == 347:
            return [sequence_items(t) for t in output["pair"]] == views
    except (KeyError, TypeError, ValueError):
        return False
    return False


def binding_cases():
    owner, other = payload(50), payload(51)
    row = lambda u, key, x, y: pair(pair(u, key), pair(x, y))
    rows = [row(owner, payload(60), payload(8), EMPTY),
            row(owner, payload(61), payload(8), payload(8)),
            row(owner, payload(62), pair(payload(8), payload(9)), pair(EMPTY, EMPTY))]
    rule = schema(pair(variable(0), pair(variable(1), pair(variable(2), variable(3)))),
                  [[0, 345, pair(variable(0), pair(variable(1), variable(2)))],
                   [1, 346, pair(variable(0), pair(variable(1), variable(3)))]])
    lib = library(347, rule)

    def goal(u, source, outputs=None, socket=0):
        if outputs is None:
            outputs = binding_views(u, source) or [[], []]
        return [socket, 347, pair(u, pair(sequence(source), pair(*(sequence(xs) for xs in outputs))))]

    def example(name, sources, goals, enabled=True):
        frontier = [call for u, bs in sources for call in binding_frontier(u, bs)]
        assert all(binding_call_true(*call) for call in frontier)
        item = case(name, lib if enabled else [], frontier, copy.deepcopy(frontier), goals, ("binding", enabled))
        item["binding_sources"] = sources
        item["native_goal_truth"] = [binding_call_true(d, t) for _, d, t in goals]
        item["semantic_boundary"] = (
            "Known traversal calls are constructed by the native row converter. The accepted exactness and closure contracts apply. "
            "This does not admit an entire symbolic argument or arbitrary paired terms as patterns.")
        return item

    same = [[owner, rows]]
    sources = [[owner, rows[:1]], [other, [row(other, payload(60), payload(8), EMPTY)]]]
    two_goals = [goal(u, bs, socket=i) for i, (u, bs) in enumerate(sources)]
    repeated = [rows[0], rows[0], rows[1]]
    wrong_owner = [row(other, payload(60), payload(8), EMPTY)]
    nonpaired = [pair(pair(owner, payload(60)), payload(8))]
    outside_pattern_range = [row(owner, payload(63), EMPTY, payload())]
    malformed = [row(owner, payload(60), payload(8), payload(256))]
    wrong_outputs = copy.deepcopy(binding_views(owner, rows))
    wrong_outputs[1][0] = pair(payload(60), payload(99))
    wrong_tail = [0, 347, pair(owner, pair(sequence(rows), pair(sequence(wrong_outputs[0]), payload(9))))]
    range_control = example("binding-conversion-without-pattern-admission",
                            [[owner, outside_pattern_range]], [goal(owner, outside_pattern_range)])
    range_control["semantic_boundary"] = (
        "The native row conversion succeeds. The observation pair (empty-artifact target, empty payload) "
        "is outside the pattern-observation image: target_valuation_payload forces a literal payload pattern, "
        "whose first observation is also that payload. No pattern or whole-argument admission follows.")
    return [
        example("binding-before-rule", same, [goal(owner, rows)], False),
        example("binding-native-report", same, [goal(owner, rows)]),
        example("binding-empty", [[owner, []]], [goal(owner, [])]),
        example("binding-equal-first-observations", [[owner, rows[:2]]], [goal(owner, rows[:2])]),
        example("binding-repeated-rows", [[owner, repeated]], [goal(owner, repeated)]),
        example("binding-different-owners", sources, two_goals),
        example("binding-reordered-repeated-sources", sources[::-1] + sources, two_goals),
        example("binding-wrong-owner", [[owner, wrong_owner]], [goal(owner, wrong_owner)]),
        example("binding-nonpaired-value", [[owner, nonpaired]], [goal(owner, nonpaired)]),
        range_control,
        example("binding-unused-malformed-component", same + [[owner, malformed]], [goal(owner, rows)]),
        example("binding-unused-malformed-context", same + [[payload(256), []]], [goal(owner, rows)]),
        example("binding-wrong-requested-output", same, [goal(owner, rows, wrong_outputs)]),
        example("binding-wrong-output-tail", same, [wrong_tail]),
        example("binding-distinct-goal-occurrences", same, [goal(owner, rows), goal(owner, rows, socket=17)])]


def variables(pattern):
    if "var" in pattern:
        return {pattern["var"]}
    return set().union(*(variables(p) for p in pattern.get("pair", [])))


def formed(pattern):
    if "payload" in pattern:
        return all(type(n) is int and 0 <= n < 256 for n in pattern["payload"])
    if "pair" in pattern:
        return all(map(formed, pattern["pair"]))
    return "var" in pattern or pattern == EMPTY


def functional(rows):
    return all(a[0] != b[0] or freeze(a[1:]) == freeze(b[1:]) for a in rows for b in rows)


def schema_formed(rule):
    premises, materials = rule["premises"], rule["materials"]
    return (formed(rule["head"]) and functional(premises) and functional(materials)
            and not ({p[0] for p in premises} & {m[0] for m in materials})
            and all(formed(p) for _, _, p in premises)
            and all(all(map(formed, fields)) for _, fields in materials))


def match(pattern, term):
    if "var" in pattern:
        return {pattern["var"]: term}
    if "pair" not in pattern:
        return {} if pattern == term and formed(term) else None
    if "pair" not in term:
        return None
    a, b = [match(p, t) for p, t in zip(pattern["pair"], term["pair"])]
    if a is None or b is None or any(a[v] != b[v] for v in a.keys() & b.keys()):
        return None
    return {**a, **b}


def evaluate(pattern, valuation):
    if "var" in pattern:
        return valuation[pattern["var"]]
    if "pair" in pattern:
        return pair(*(evaluate(p, valuation) for p in pattern["pair"]))
    return pattern


def schema_key(rule):
    return (freeze(rule["head"]), frozenset(map(freeze, rule["premises"])),
            frozenset(map(freeze, rule["materials"])))


def application_key(application):
    return (application["entry"], schema_key(application["schema"]), freeze(application["conclusion"]),
            frozenset(map(freeze, application["bindings"])), frozenset(map(freeze, application["premises"])))


def schema_variables(rule):
    return (variables(rule["head"])
            | set().union(*(variables(p) for _, _, p in rule["premises"]))
            | set().union(*(variables(p) for _, fields in rule["materials"] for p in fields)))


def reconstruct_binding_gaps(item):
    return [{**e,
             "premise_missing": sorted(schema_variables(e["schema"]) -
                 set().union(*(variables(p) for _, _, p in e["schema"]["premises"]))),
             "head_missing": sorted(schema_variables(e["schema"]) - variables(e["schema"]["head"]))}
            for e in item["library"]]


def library_entry_formed(entry):
    return (schema_formed(entry["schema"]) and set(map(freeze, entry["enumeration"]))
            == set(map(freeze, entry["schema"]["premises"])))


def application_from_valuation(entry, values):
    """Check the complete original scope and the declared empty-material family."""
    rule = entry["schema"]
    if set(values) != schema_variables(rule) or not all(map(formed, values.values())):
        return None
    if any([evaluate(p, values) for p in fields] != [EMPTY] * 5 for _, fields in rule["materials"]):
        return None
    return {"entry": entry["entry"], "schema": rule, "conclusion": evaluate(rule["head"], values),
            "bindings": [[v, values[v]] for v in sorted(values)],
            "premises": [[s, d, evaluate(p, values)] for s, d, p in rule["premises"]]}


def observation_valuations(scope, contributions, *, complete=True):
    """Enumerate complete valuations and check that whole compatible matches cover them."""
    domains = {v: {} for v in scope}
    for part in contributions:
        for v, value in part.items():
            assert v in scope and formed(value)
            domains[v][freeze(value)] = value
    ordered = sorted(scope)
    choices = [list(domains[v].values()) + ([] if complete else [None]) for v in ordered]
    for values in itertools.product(*choices):
        valuation = {v: value for v, value in zip(ordered, values) if value is not None}
        covered = set().union(*(part.keys() for part in contributions
                              if all(valuation.get(v) == value for v, value in part.items())))
        if covered == set(valuation):
            yield valuation


MATERIAL_OBSERVATION_FIELDS = ["source", "atoms", "edges", "counts", "functions"]


def schema_observation_sources(rule):
    return ([[["head"], rule["head"]]]
            + [[["premise", s, d], p] for s, d, p in rule["premises"]]
            + [[["material", field, s], p] for s, fields in rule["materials"]
               for field, p in zip(MATERIAL_OBSERVATION_FIELDS, fields)])


def schema_call_observations(entry, frontier, requests):
    return ([[["head"], t] for d, t in requests if d == entry["entry"]]
            + [[["premise", s, d], t] for s, d, _ in entry["schema"]["premises"]
               for e, t in frontier if d == e])


def recover_observations(sources, observations):
    recovered = []
    for i, p in sources:
        for j, t in observations:
            part = match(p, t) if i == j and formed(t) else None
            if part is not None:
                recovered.append((i, p, t, part))
    return recovered


def reconstruct_observed_applications(item, requests):
    applications = []
    for entry in item["library"]:
        if not library_entry_formed(entry):
            continue
        rule = entry["schema"]
        observations = schema_call_observations(entry, item["frontier"], requests)
        contributions = [part for _, _, _, part in recover_observations(schema_observation_sources(rule), observations)]
        for values in observation_valuations(schema_variables(rule), contributions):
            application = application_from_valuation(entry, values)
            if application is not None:
                applications.append(application)
    return applications


def reconstruct_applications(item):
    applications = []
    available = {freeze(c) for c in item["frontier"]}
    valid_library = True
    for entry in item["library"]:
        rule = entry["schema"]
        valid = library_entry_formed(entry)
        valid_library &= valid
        if not valid:
            continue
        scope = schema_variables(rule)
        domains = {v: {} for v in scope}
        for _, d, p in rule["premises"]:
            for e, t in item["frontier"]:
                matched = match(p, t) if d == e else None
                if matched is not None:
                    for v, value in matched.items():
                        if formed(value):
                            domains[v][freeze(value)] = value
        ordered = sorted(scope)
        # Enumerate complete valuations, independently of the executor's
        # ordered recursive premise joins, then check every original premise.
        for values in itertools.product(*(domains[v].values() for v in ordered)):
            valuation = dict(zip(ordered, values))
            premises = [[s, d, evaluate(p, valuation)] for s, d, p in rule["premises"]]
            if any(freeze([d, t]) not in available for _, d, t in premises):
                continue
            application = application_from_valuation(entry, valuation)
            if application is not None:
                applications.append(application)
    return applications, valid_library


def reconstruction_frontiers(item):
    frontier = {freeze(q): q for q in item["frontier"]}
    stages = [list(frontier.values())]
    for _ in range(item.get("construction_rounds", 0)):
        applications, _ = reconstruct_applications({**item, "frontier": stages[-1]})
        for application in applications:
            q = [application["entry"], application["conclusion"]]
            frontier[freeze(q)] = q
        stages.append(list(frontier.values()))
    return stages


def reconstruct_requested_applications(item, requests):
    applications = []
    for e in item["library"]:
        rule = e["schema"]
        if not library_entry_formed(e):
            continue
        for d, t in requests:
            if d != e["entry"]:
                continue
            values = match(rule["head"], t)
            if values is not None:
                application = application_from_valuation(e, values)
                if application is not None:
                    applications.append(application)
    return applications


def reconstruction_guided_states(item):
    frontier = {freeze(q): q for q in item["frontier"]}
    requests = {freeze(g[1:]): g[1:] for g in item["goals"]}
    applications = {}
    states = [{"frontier": list(frontier.values()), "requests": list(requests.values()), "applications": []}]
    for _ in range(item["guided_rounds"] + 1):
        new = reconstruct_observed_applications({**item, "frontier": list(frontier.values())},
                                               list(requests.values()))
        for a in new:
            applications[application_key(a)] = a
            q = [a["entry"], a["conclusion"]]
            frontier[freeze(q)] = q
            for _, d, t in a["premises"]:
                requests[freeze([d, t])] = [d, t]
        states.append({"frontier": list(frontier.values()), "requests": list(requests.values()),
                       "applications": list(applications.values())})
    return states


def reconstruct(item, *, guided_states=None, frontiers=None):
    if "guided_rounds" in item:
        states = guided_states if guided_states is not None else reconstruction_guided_states(item)
        item = {**item, "frontier": states[-1]["frontier"]}
        applications = states[-1]["applications"]
        valid_library = all(schema_formed(e["schema"]) and set(map(freeze, e["enumeration"])) ==
                            set(map(freeze, e["schema"]["premises"])) for e in item["library"])
    else:
        stages = frontiers if frontiers is not None else reconstruction_frontiers(item)
        item = {**item, "frontier": stages[-1]}
        applications, valid_library = reconstruct_applications(item)
    rules = {(freeze([a["entry"], a["conclusion"]]), frozenset(map(freeze, a["premises"]))) for a in applications}
    known = set(map(freeze, item["known"]))
    reached, demand, reasons = reconstruct_inference(rules, known, {freeze([d, t]) for _, d, t in item["goals"]})
    source_formed = all(formed(u) and all(map(formed, rows)) for u, rows in item.get("binding_sources", []))
    source_formed &= all(map(formed, item.get("construction_source", [])))
    return {"input_formed": source_formed and valid_library and functional(item["goals"])
            and all(formed(t) for _, t in item["frontier"] + item["known"])
            and all(formed(t) for _, _, t in item["goals"]),
            "applications": set(map(application_key, applications)),
            "residual": {freeze(g) for g in item["goals"] if freeze(g[1:]) not in reached},
            "demand": demand, "reasons": reasons}


def reconstruct_inference(rules, known, targets):
    """Independent finite conjunctive closure and complete backward demand."""
    rules = {(q, h) for q, h in rules if functional(h)}
    reached = set(known)
    while True:
        more = {q for q, h in rules if {row[1:] if len(row) > 2 else row[1] for row in h} <= reached}
        if more <= reached:
            break
        reached |= more
    demand = set(targets)
    while True:
        more = {row[1:] if len(row) > 2 else row[1] for q, h in rules if q in demand - known for row in h}
        if more <= demand:
            break
        demand |= more
    reasons = {(q, h, row[0], row[1:] if len(row) > 2 else row[1])
               for q, h in rules if q in demand - known for row in h}
    return reached, demand, reasons


def reconstruct_coverage(item):
    rules = {(entry["entry"], frozenset((s, d) for s, d, _ in entry["schema"]["premises"]))
             for entry in item["library"]}
    reached, demand, reasons = reconstruct_inference(rules, {d for d, _ in item["known"]},
                                                    {d for _, d, _ in item["goals"]})
    library_formed = all(schema_formed(e["schema"]) and
                        set(map(freeze, e["enumeration"])) == set(map(freeze, e["schema"]["premises"]))
                        for e in item["library"])
    return {"input_formed": library_formed and functional(item["goals"])
            and all(formed(t) for _, t in item["known"]) and all(formed(t) for _, _, t in item["goals"]),
            "rules": rules, "blocked": {freeze(g) for g in item["goals"] if g[1] not in reached},
            "demand": demand, "reasons": reasons}


def ml_term(term, pattern=False):
    if "var" in term:
        return "N.Finite_Variable (" + investigate.ml_nat(term["var"]) + ")"
    if "pair" in term:
        return "N.Finite_" + ("Pattern_" if pattern else "") + "Pair (" + ",".join(ml_term(t, pattern) for t in term["pair"]) + ")"
    if term == EMPTY:
        return "N.Finite_" + ("Pattern_" if pattern else "") + "Target emptyTarget"
    return "N.Finite_" + ("Pattern_" if pattern else "") + "Payload " + investigate.ml_list(term["payload"], investigate.ml_nat)


def ml_premise(row, pattern=False):
    s, d, t = row
    return "(" + investigate.ml_nat(s) + ",(" + investigate.ml_nat(d) + "," + ml_term(t, pattern) + "))"


def ml_call(row):
    return "(" + investigate.ml_nat(row[0]) + "," + ml_term(row[1]) + ")"


def ml_binding_source(source):
    owner, rows = source
    return "(" + ml_term(owner) + "," + investigate.ml_list(rows, ml_term) + ")"


def ml_schema(rule, *, schema_constructor="N.make", material_constructor="N.makea"):
    premises = investigate.ml_list(rule["premises"], lambda r: ml_premise(r, True))
    materials = investigate.ml_list(rule["materials"], lambda r: "(" + investigate.ml_nat(r[0]) + "," + material_constructor + " "
                                    + " ".join("(" + ml_term(p, True) + ")" for p in r[1]) + ")")
    return schema_constructor + " (" + ml_term(rule["head"], True) + ") (fs " + premises + ") (fs " + materials + ")"


def ml_library_source(entry):
    return "(" + investigate.ml_nat(entry["entry"]) + ",(" + ml_schema(entry["schema"]) + "))"


def ml_library(entry):
    return "(" + investigate.ml_nat(entry["entry"]) + ",(" + ml_schema(entry["schema"]) + "," + investigate.ml_list(entry["enumeration"], lambda r: ml_premise(r, True)) + "))"


def ml_native_libraries(selections):
    parts = []
    for selection in selections:
        name = selection["name"]
        assert name.isidentifier() and name.isascii(), "Expected an exported Standard ML identifier."
        source = "N." + name
        if "entries" in selection:
            entries = selection["entries"]
            assert all(type(d) is int and d >= 0 for d in entries)
            allowed = investigate.ml_list(entries, lambda d: "(n " + str(d) + ")")
            source = "(List.filter (fn (d,_) => List.exists (fn e => d=e) " + allowed + ") " + source + ")"
        parts.append(source)
    return "(" + " @ ".join(parts) + ")" if parts else "[]"


def ml_case_library(item):
    assert not ("native_libraries" in item and "compilation_sources" in item)
    if "native_libraries" in item:
        return ml_native_libraries(item["native_libraries"])
    if "compilation_sources" in item:
        return "(N.natural_compiled_library " + investigate.ml_list(item["compilation_sources"], ml_library_source) + ")"
    return investigate.ml_list(item["library"], ml_library)


SCALAR_JSON_PRELUDE = r'''fun jnat x = IntInf.toString (N.integer_of_nat x);
fun jlist f xs = "[" ^ String.concatWith "," (map f xs) ^ "]";
'''

TERM_JSON_PRELUDE = r'''fun jterm (N.Finite_Payload b) = "{\"payload\":" ^ jlist jnat b ^ "}"
 | jterm (N.Finite_Pair (a,b)) = "{\"pair\":[" ^ jterm a ^ "," ^ jterm b ^ "]}"
 | jterm (N.Finite_Target t) = jtarget t;
'''

FSET_TERM_SCHEMA_JSON_PRELUDE = r'''val n = N.nat_of_integer;
val fs = N.fset_of_list;
val emptyTarget = N.Finite_Whole N.finite_empty_artifact;
val emptyTerm = N.Finite_Target emptyTarget;
''' + SCALAR_JSON_PRELUDE + r'''fun elements a = case N.fset a of N.Set xs => xs | N.Coset _ => raise Fail "Nonfinite representation";
fun jf f a = jlist f (elements a);
fun jtarget t = if N.finite_reasoning_term_equal (N.Finite_Target t) emptyTerm
 then "{\"empty_artifact\":true}" else raise Fail "Unexpected artifact outside the complete declared control family";
''' + TERM_JSON_PRELUDE + r'''fun jpattern (N.Finite_Variable v) = "{\"var\":" ^ jnat v ^ "}"
 | jpattern (N.Finite_Pattern_Payload b) = "{\"payload\":" ^ jlist jnat b ^ "}"
 | jpattern (N.Finite_Pattern_Pair (a,b)) = "{\"pair\":[" ^ jpattern a ^ "," ^ jpattern b ^ "]}"
 | jpattern (N.Finite_Pattern_Target t) = jtarget t;
fun jcall (d,t) = "[" ^ jnat d ^ "," ^ jterm t ^ "]";
fun jbinding (v,t) = "[" ^ jnat v ^ "," ^ jterm t ^ "]";
fun jpremise (s,(d,t)) = "[" ^ jnat s ^ "," ^ jnat d ^ "," ^ jterm t ^ "]";
fun jsymbolic (s,(d,p)) = "[" ^ jnat s ^ "," ^ jnat d ^ "," ^ jpattern p ^ "]";
fun jmaterial (s,m) = "[" ^ jnat s ^ "," ^ jlist jpattern
 [N.finite_material_source m,N.finite_material_atoms m,N.finite_material_edges m,
  N.finite_material_counts m,N.finite_material_functions m] ^ "]";
fun jschema s = "{\"head\":" ^ jpattern (N.finite_schema_conclusion s) ^
 ",\"premises\":" ^ jf jsymbolic (N.finite_schema_premises s) ^
 ",\"materials\":" ^ jf jmaterial (N.finite_schema_materials s) ^ "}";
fun jlibrary (d,(s,ps)) = "{\"entry\":" ^ jnat d ^ ",\"schema\":" ^ jschema s ^
 ",\"enumeration\":" ^ jlist jsymbolic ps ^ "}";
'''


LOCAL_APPLICATION_JSON_PRELUDE = r'''fun jlocal (t,(v,h)) = "{\"conclusion\":" ^ jterm t ^ ",\"bindings\":" ^ jf jbinding v ^
 ",\"premises\":" ^ jf jpremise h ^ "}";
'''


PRELUDE = r'''
structure N = Native_Reasoning;
''' + FSET_TERM_SCHEMA_JSON_PRELUDE + r'''fun emitCompiledLibrary id rows = print
 ("COMPILED_LIBRARY " ^ Int.toString id ^ " " ^ jlist jlibrary rows ^ "\n");
fun japplication (d,(s,(t,(v,h)))) = "{\"entry\":" ^ jnat d ^ ",\"schema\":" ^ jschema s ^
 ",\"conclusion\":" ^ jterm t ^ ",\"bindings\":" ^ jf jbinding v ^ ",\"premises\":" ^ jf jpremise h ^ "}";
fun jreason (q,(h,(s,p))) = "{\"conclusion\":" ^ jcall q ^ ",\"premises\":" ^ jf jpremise h ^
 ",\"premise\":" ^ jnat s ^ ",\"condition\":" ^ jcall p ^ "}";
fun emit id (formed,(applications,(residual,(demand,reasons)))) = print
 ("REASONING_RESULT " ^ Int.toString id ^ " {\"input_formed\":" ^ Bool.toString formed ^
 ",\"applications\":" ^ jf japplication applications ^ ",\"residual\":" ^ jf jpremise residual ^
 ",\"demand\":" ^ jf jcall demand ^ ",\"reasons\":" ^ jf jreason reasons ^ "}\n");
fun emitViews id index (u,rows) = let
 val outputs = case N.finite_binding_observation_outputs u rows of NONE => "null"
  | SOME (xs,ys) => "[" ^ jlist jterm xs ^ "," ^ jlist jterm ys ^ "]";
 val frontier = N.finite_binding_observation_frontier u rows
 in print ("BINDING_RESULT " ^ Int.toString id ^ " " ^ Int.toString index ^
  " {\"outputs\":" ^ outputs ^ ",\"frontier\":" ^ jlist jcall frontier ^ "}\n") end;
fun emitConstructionFrontier id index library initial = print
 ("CONSTRUCTION_FRONTIER " ^ Int.toString id ^ " " ^ Int.toString index ^ " " ^
  jf jcall (N.natural_construction_frontier (n (IntInf.fromInt index)) library (fs initial)) ^ "\n");
fun emitConstructionSeed id u row = print
 ("CONSTRUCTION_SEED " ^ Int.toString id ^ " " ^ jlist jcall (N.finite_binding_list_seed u row) ^ "\n");
fun jinstruction (N.Pair_Admission_Instruction (d,a,b)) = "[\"pair\"," ^ jnat d ^ "," ^ jnat a ^ "," ^ jnat b ^ "]"
 | jinstruction (N.List_Admission_Instruction (d,a)) = "[\"list\"," ^ jnat d ^ "," ^ jnat a ^ "]";
fun emitInput start = let val (admitted,((root,(next,instructions)),decisions)) = N.generated_input_execution (n start)
 in print ("INPUT_RESULT " ^ IntInf.toString start ^ " {\"domain_admitted\":" ^ Bool.toString admitted ^
 ",\"root\":" ^ jnat root ^ ",\"next\":" ^ jnat next ^ ",\"instructions\":" ^ jlist jinstruction instructions ^
 ",\"decisions\":" ^ jlist Bool.toString decisions ^ "}\n") end;
'''


GUIDED_PRELUDE = r'''
fun emitGuidedState id index library initial requested = let
 val (c,(q,a)) = N.natural_guided_state (n (IntInf.fromInt index)) library (fs initial) (fs requested)
 in print ("GUIDED_STATE " ^ Int.toString id ^ " " ^ Int.toString index ^
 " {\"frontier\":" ^ jf jcall c ^ ",\"requests\":" ^ jf jcall q ^
 ",\"applications\":" ^ jf japplication a ^ "}\n") end;
'''


ENVIRONMENT_REQUIREMENTS_PRELUDE = r'''
fun emitEnvironmentRequirements id requirements = let
 fun row (g,e) = "{\"goal\":" ^ jpremise g ^ ",\"environment\":" ^ jterm e ^
   ",\"possible_outer_shape\":" ^ Bool.toString (N.finite_environment_outer_shape e) ^ "}"
 in print ("ENVIRONMENT_REQUIREMENT_RESULT " ^ Int.toString id ^ " " ^ jlist row requirements ^ "\n") end;
'''


BINDING_GAPS_PRELUDE = r'''
fun jbindinggaps (d,(s,ps)) = let
 val (premise,head) = N.natural_schema_binding_gaps s
 in "{\"entry\":" ^ jnat d ^ ",\"schema\":" ^ jschema s ^
 ",\"enumeration\":" ^ jlist jsymbolic ps ^ ",\"premise_missing\":" ^ jf jnat premise ^
 ",\"head_missing\":" ^ jf jnat head ^ "}" end;
fun emitBindingGaps id rows = print
 ("BINDING_GAPS_RESULT " ^ Int.toString id ^ " " ^ jlist jbindinggaps rows ^ "\n");
'''


COVERAGE_PRELUDE = r'''
fun jentryPremise (s,d) = "[" ^ jnat s ^ "," ^ jnat d ^ "]";
fun jentryRule (d,h) = "[" ^ jnat d ^ "," ^ jf jentryPremise h ^ "]";
fun jentryReason (d,(h,(s,e))) = "[" ^ jnat d ^ "," ^ jf jentryPremise h ^
 "," ^ jnat s ^ "," ^ jnat e ^ "]";
fun emitCoverage id (formed,(rules,(blocked,(demand,reasons)))) = print
 ("COVERAGE_RESULT " ^ Int.toString id ^ " {\"input_formed\":" ^ Bool.toString formed ^
 ",\"rules\":" ^ jf jentryRule rules ^ ",\"blocked\":" ^ jf jpremise blocked ^
 ",\"demand\":" ^ jf jnat demand ^ ",\"reasons\":" ^ jf jentryReason reasons ^ "}\n");
'''


def normalized_reasoning_report(actual):
    """Recover every report field at its declared finite-set boundaries."""
    return {**actual,
            "applications": set(map(application_key, actual["applications"])),
            "residual": set(map(freeze, actual["residual"])),
            "demand": set(map(freeze, actual["demand"])),
            "reasons": {(freeze(r["conclusion"]), frozenset(map(freeze, r["premises"])),
                         r["premise"], freeze(r["condition"])) for r in actual["reasons"]}}


def run(args, *, cases_factory=cases, cases_source=None, cases_dependencies=(), library_exports=(),
        known_call_contract=None):
    if not __debug__:
        raise ValueError("Execution review requires Python assertions to be enabled.")
    output = args.output.resolve()
    if output.exists():
        raise ValueError("The execution output already exists; retain it and use a new directory.")
    output.mkdir(parents=True)
    report = {"status": "failed", "invocation": str(uuid.uuid4()), "scope": "Complete finite reports for the supplied case family, with native comparisons where declared"}
    try:
        if known_call_contract is None:
            known_call_contract = KnownCallContract(binding_call_true,
                "Exact binding-observation row and list conditions for native entries 343 through 347")
        if not isinstance(known_call_contract, KnownCallContract):
            raise ValueError("Expected an explicit KnownCallContract.")
        report["known_call_contract"] = known_call_contract.record()
        proof_path = args.proof.resolve()
        proof, engine, source_files = proved_code.proved_export(proof_path, args.engine, project=ROOT)
        if cases_factory is not cases:
            assert cases_source is not None, "A separate case family requires its source provenance."
        inputs = cases_factory()
        write_json(output / "cases.json", inputs)
        program = "use " + investigate.ml_string(str(engine)) + ";\n" + PRELUDE
        if any(item.get("environment_requirements") for item in inputs):
            assert "Factor_Reader_Source_Requirements" in proof["sources"]
            program += ENVIRONMENT_REQUIREMENTS_PRELUDE
        if any(item.get("diagnose_coverage") for item in inputs):
            program += COVERAGE_PRELUDE
        if any(item.get("diagnose_bindings") for item in inputs):
            program += BINDING_GAPS_PRELUDE
        if any("guided_rounds" in item for item in inputs):
            program += GUIDED_PRELUDE
        for i, item in enumerate(inputs):
            operation = item["operation"]
            selected_library = ml_case_library(item)
            if "native_libraries" in item or "compilation_sources" in item:
                program += "val () = emitCompiledLibrary " + str(i) + " " + selected_library + ";\n"
            if operation and operation[0] == "binding":
                call = ("N.binding_observation_investigation " + str(operation[1]).lower() + " "
                        + investigate.ml_list(item["binding_sources"], ml_binding_source)
                        + " (fs " + investigate.ml_list(item["goals"], ml_premise) + ")")
            elif operation and operation[0] in {"chain", "native_chain"}:
                rounds = operation[1]
                assert type(rounds) is int and rounds >= 0 and rounds == item["construction_rounds"]
                if operation[0] == "native_chain":
                    owner, row = item["construction_source"]
                    # The native wrapper specializes its own library. The
                    # separate frontier probe receives the explicit finite
                    # specification, checked against every returned schema
                    # and premise in the native wrapper's complete report.
                    lib = investigate.ml_list(item["library"], ml_library)
                    frontier = "(N.finite_binding_list_seed (" + ml_term(owner) + ") (" + ml_term(row) + "))"
                    call = ("N.binding_list_construction_investigation (n " + str(rounds) + ") "
                            + str(operation[2]).lower() + " " + str(operation[3]).lower()
                            + " (" + ml_term(owner) + ") (" + ml_term(row) + ")")
                    program += "val () = emitConstructionSeed " + str(i) + " (" + ml_term(owner) + ") (" + ml_term(row) + ");\n"
                else:
                    lib = selected_library
                    frontier = investigate.ml_list(item["frontier"], ml_call)
                    driver = ("N.inference_reader_investigation (n " + str(rounds) + ")"
                              if item.get("native_reader_investigation") else
                              "N.natural_chained_investigation (n " + str(rounds) + ") " + lib)
                    call = (driver + " " + frontier
                            + " (fs " + investigate.ml_list(item["known"], ml_call) + ")")
                call += " (fs " + investigate.ml_list(item["goals"], ml_premise) + ")"
                for j in range(rounds + 1):
                    program += "val () = emitConstructionFrontier " + str(i) + " " + str(j) + " (" + lib + ") " + frontier + ";\n"
            elif operation and operation[0] == "guided":
                rounds = operation[1]
                assert type(rounds) is int and rounds >= 0 and rounds == item["guided_rounds"]
                frontier = investigate.ml_list(item["frontier"], ml_call)
                driver = ("N.binding_observation_guided_investigation (n " + str(rounds) + ") "
                          + str(item["native_binding_guided"]).lower() if "native_binding_guided" in item else
                          "N.natural_guided_investigation (n " + str(rounds) + ") " + selected_library)
                call = (driver + " " + frontier
                        + " (fs " + investigate.ml_list(item["known"], ml_call) + ")"
                        + " (fs " + investigate.ml_list(item["goals"], ml_premise) + ")")
                requested = investigate.ml_list([g[1:] for g in item["goals"]], ml_call)
                for j in range(rounds + 2):
                    program += ("val () = emitGuidedState " + str(i) + " " + str(j) + " " + selected_library
                                + " " + frontier + " " + requested + ";\n")
            elif operation and operation[0] != "report":
                call = ("N.admission_pair_reasoning_experiment (n " + str(operation[1]) + ")" if operation[0] == "stage"
                        else "N.reasoning_counterexample_experiment " + str(operation[1]).lower())
            else:
                call = ("N.specialization_report_investigation" if operation else
                        "N.natural_learned_investigation " + investigate.ml_list(item["library"], ml_library))
                call += " " + investigate.ml_list(item["frontier"], ml_call)
                call += " (fs " + investigate.ml_list(item["known"], ml_call) + ")"
                call += " (fs " + investigate.ml_list(item["goals"], ml_premise) + ")"
            for j, source in enumerate(item.get("binding_sources", [])):
                program += "val () = emitViews " + str(i) + " " + str(j) + " " + ml_binding_source(source) + ";\n"
            program += "val () = emit " + str(i) + " (" + call + ");\n"
            if item.get("diagnose_coverage"):
                coverage_call = ("N.reader_library_coverage" if item.get("native_reader_coverage") else
                                 "N.natural_library_coverage " + (ml_native_libraries(item["native_libraries"])
                                  if "native_libraries" in item else investigate.ml_list(item["library"], ml_library)))
                program += ("val () = emitCoverage " + str(i) + " (" + coverage_call
                            + " (fs " + investigate.ml_list(item["known"], ml_call) + ")"
                            + " (fs " + investigate.ml_list(item["goals"], ml_premise) + "));\n")
            if item.get("diagnose_bindings"):
                program += "val () = emitBindingGaps " + str(i) + " " + selected_library + ";\n"
            if item.get("environment_requirements"):
                requests = item["environment_requirements"]
                for row in requests:
                    values = match(row["source_schema"]["head"], row["goal"][2])
                    assert values is not None and values[row["source_variable"]] == row["environment"]
                    assert row["goal"] in item["goals"]
                encoded = investigate.ml_list(requests, lambda row:
                    "(" + ml_premise(row["goal"]) + "," + ml_term(row["environment"]) + ")")
                program += "val () = emitEnvironmentRequirements " + str(i) + " " + encoded + ";\n"
        assert len(set(library_exports)) == len(library_exports)
        for name in library_exports:
            assert name.isidentifier() and name.isascii(), "Expected an exported Standard ML identifier."
            program += ('val () = print ("LIBRARY_CATALOG ' + name + ' " ^ jlist jlibrary N.'
                        + name + ' ^ "\\n");\n')
        starts = [0, 335, 336, 337, 1000, 10**25]
        program += "val () = List.app emitInput [" + ",".join(map(str, starts)) + "];\n"
        runtime = output / "execute.ML"
        runtime.write_text(program)
        tracked = [proof_path, engine, args.poly.resolve(), Path(__file__).resolve(), Path(investigate.__file__).resolve(),
                   runtime, output / "cases.json"]
        tracked.extend(loaded_tool_sources())
        tracked.extend(known_call_contract.inputs())
        if cases_source is not None:
            tracked.append(Path(cases_source).resolve())
        tracked.extend(Path(p).resolve() for p in cases_dependencies)
        hashes = {str(p): digest(p) for p in tracked} | source_files
        write_json(output / "execution-inputs.json", hashes)
        proved_code.archive_execution_inputs(output, report, hashes, external=[args.poly])
        with (output / "results.log").open("w") as log:
            completed = subprocess.run([str(args.poly.resolve()), "--script", str(runtime)], stdout=log,
                                       stderr=subprocess.STDOUT, timeout=180, check=False)
        assert completed.returncode == 0, "The generated module or runtime serializer failed; inspect results.log."
        seen, seen_starts, seen_views, seen_frontiers, seen_seeds, seen_coverage, seen_compilations, findings = set(), set(), set(), set(), set(), set(), set(), []
        exported_libraries = {}
        seen_binding_gaps = set()
        seen_guided_states = set()
        seen_environment_requirements = set()
        expected_guided = {i: reconstruction_guided_states(item) for i, item in enumerate(inputs) if "guided_rounds" in item}
        expected_frontiers = {i: reconstruction_frontiers(item) for i, item in enumerate(inputs) if "guided_rounds" not in item}
        expected_reports = {i: reconstruct(item, guided_states=expected_guided.get(i), frontiers=expected_frontiers.get(i))
                            for i, item in enumerate(inputs)}
        for line in (output / "results.log").read_text().splitlines():
            if line.startswith("REASONING_RESULT "):
                identifier, raw = line.removeprefix("REASONING_RESULT ").split(" ", 1)
                index = int(identifier)
                assert index not in seen and 0 <= index < len(inputs)
                seen.add(index)
                actual = json.loads(raw)
                expected = expected_reports[index]
                normalized = normalized_reasoning_report(actual)
                mismatch = [key for key in expected if expected[key] != normalized[key]]
                assert not mismatch, (inputs[index]["name"], mismatch)
                check_expected_report(inputs[index], actual)
                findings.append({"case": inputs[index]["name"], "formed": actual["input_formed"],
                                 "applications": len(normalized["applications"]), "residual": len(normalized["residual"]),
                                 "demand": len(normalized["demand"]), "reasons": len(normalized["reasons"]),
                                 "checked": list(expected)})
                if "native_goal_truth" in inputs[index]:
                    findings[-1]["known_calls_checked"] = known_call_contract.validate(inputs[index]["known"])
                    assert all(truth or freeze(goal) in normalized["residual"]
                               for goal, truth in zip(inputs[index]["goals"], inputs[index]["native_goal_truth"]))
                    findings[-1]["native_goal_truth"] = inputs[index]["native_goal_truth"]
            elif line.startswith("GUIDED_STATE "):
                identifier, stage, raw = line.removeprefix("GUIDED_STATE ").split(" ", 2)
                index, step = int(identifier), int(stage)
                assert index in expected_guided and 0 <= step < len(expected_guided[index])
                assert (index, step) not in seen_guided_states
                actual = json.loads(raw)
                expected = expected_guided[index][step]
                for field in ["frontier", "requests"]:
                    assert set(map(freeze, actual[field])) == set(map(freeze, expected[field])), (index, step, field)
                assert set(map(application_key, actual["applications"])) == set(map(application_key, expected["applications"]))
                seen_guided_states.add((index, step))
            elif line.startswith("BINDING_GAPS_RESULT "):
                identifier, raw = line.removeprefix("BINDING_GAPS_RESULT ").split(" ", 1)
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_binding_gaps
                assert inputs[index].get("diagnose_bindings")
                actual = json.loads(raw)
                def gap_key(e):
                    return (e["entry"], schema_key(e["schema"]), tuple(map(freeze, e["enumeration"])),
                            frozenset(e["premise_missing"]), frozenset(e["head_missing"]))
                assert list(map(gap_key, actual)) == list(map(gap_key, reconstruct_binding_gaps(inputs[index])))
                seen_binding_gaps.add(index)
            elif line.startswith("ENVIRONMENT_REQUIREMENT_RESULT "):
                identifier, raw = line.removeprefix("ENVIRONMENT_REQUIREMENT_RESULT ").split(" ", 1)
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_environment_requirements
                requests = inputs[index].get("environment_requirements")
                assert requests
                expected = [{"goal": row["goal"], "environment": row["environment"],
                             "possible_outer_shape": "pair" in row["environment"]} for row in requests]
                assert json.loads(raw) == expected
                seen_environment_requirements.add(index)
            elif line.startswith("LIBRARY_CATALOG "):
                name, raw = line.removeprefix("LIBRARY_CATALOG ").split(" ", 1)
                assert name in library_exports and name not in exported_libraries
                entries = json.loads(raw)
                assert all(schema_formed(e["schema"]) and
                           set(map(freeze, e["enumeration"])) == set(map(freeze, e["schema"]["premises"]))
                           for e in entries), (name, "incomplete or malformed exported library")
                catalog = output / ("library-" + name + ".json")
                write_json(catalog, entries)
                exported_libraries[name] = {"path": str(catalog), "sha256": digest(catalog),
                                            "entries": [e["entry"] for e in entries]}
            elif line.startswith("COMPILED_LIBRARY "):
                identifier, raw = line.removeprefix("COMPILED_LIBRARY ").split(" ", 1)
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_compilations
                assert "compilation_sources" in inputs[index] or "native_libraries" in inputs[index]
                actual = json.loads(raw)
                expected = inputs[index]["library"]
                key = lambda e: (e["entry"], schema_key(e["schema"]), tuple(map(freeze, e["enumeration"])))
                assert list(map(key, actual)) == list(map(key, expected)), (inputs[index]["name"], "executed library")
                seen_compilations.add(index)
            elif line.startswith("COVERAGE_RESULT "):
                identifier, raw = line.removeprefix("COVERAGE_RESULT ").split(" ", 1)
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_coverage and inputs[index].get("diagnose_coverage")
                actual = json.loads(raw)
                normalized = {"input_formed": actual["input_formed"],
                              "rules": {(d, frozenset(map(tuple, h))) for d, h in actual["rules"]},
                              "blocked": set(map(freeze, actual["blocked"])), "demand": set(actual["demand"]),
                              "reasons": {(d, frozenset(map(tuple, h)), s, e) for d, h, s, e in actual["reasons"]}}
                assert normalized == reconstruct_coverage(inputs[index]), (inputs[index]["name"], "coverage")
                assert normalized["blocked"] <= expected_reports[index]["residual"], (inputs[index]["name"], "unsound obstruction")
                seen_coverage.add(index)
            elif line.startswith("BINDING_RESULT "):
                identifier, source_id, raw = line.removeprefix("BINDING_RESULT ").split(" ", 2)
                index, source_index = int(identifier), int(source_id)
                assert 0 <= index < len(inputs) and (index, source_index) not in seen_views
                assert 0 <= source_index < len(inputs[index]["binding_sources"])
                owner, rows = inputs[index]["binding_sources"][source_index]
                actual = json.loads(raw)
                assert actual == {"outputs": binding_views(owner, rows), "frontier": binding_frontier(owner, rows)}
                assert all(binding_call_true(*call) for call in actual["frontier"])
                seen_views.add((index, source_index))
            elif line.startswith("CONSTRUCTION_FRONTIER "):
                identifier, round_id, raw = line.removeprefix("CONSTRUCTION_FRONTIER ").split(" ", 2)
                index, step = int(identifier), int(round_id)
                assert 0 <= index < len(inputs) and (index, step) not in seen_frontiers
                expected = expected_frontiers[index]
                assert 0 <= step < len(expected)
                assert set(map(freeze, json.loads(raw))) == set(map(freeze, expected[step]))
                seen_frontiers.add((index, step))
            elif line.startswith("CONSTRUCTION_SEED "):
                identifier, raw = line.removeprefix("CONSTRUCTION_SEED ").split(" ", 1)
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_seeds
                actual = json.loads(raw)
                owner, row = inputs[index]["construction_source"]
                views = binding_views(owner, [row])
                expected = [] if views is None else [
                    [343, pair(owner, pair(row, views[0][0]))],
                    [345, pair(owner, pair(payload(), payload()))]]
                assert actual == expected == inputs[index]["frontier"]
                assert all(binding_call_true(*call) for call in actual)
                seen_seeds.add(index)
            elif line.startswith("INPUT_RESULT "):
                identifier, raw = line.removeprefix("INPUT_RESULT ").split(" ", 1)
                start = int(identifier)
                assert start in starts and start not in seen_starts
                seen_starts.add(start)
                actual = json.loads(raw)
                expected = {"domain_admitted": start >= 336, "root": start + 2, "next": start + 3,
                    "instructions": [["pair", start, 2, 2], ["list", start + 1, start], ["pair", start + 2, 311, start + 1]],
                    "decisions": [i not in {6, 7, 8, 10} for i in range(11)] if start >= 336 else []}
                assert actual == expected, ("installed input", start)
        assert len(seen) == len(inputs) and seen_starts == set(starts)
        assert seen_views == {(i, j) for i, item in enumerate(inputs) for j in range(len(item.get("binding_sources", [])))}
        assert seen_frontiers == {(i, j) for i, item in enumerate(inputs) if "construction_rounds" in item
                                  for j in range(item["construction_rounds"] + 1)}
        assert seen_seeds == {i for i, item in enumerate(inputs) if "construction_source" in item}
        assert seen_coverage == {i for i, item in enumerate(inputs) if item.get("diagnose_coverage")}
        assert seen_compilations == {i for i, item in enumerate(inputs)
                                     if "compilation_sources" in item or "native_libraries" in item}
        assert seen_binding_gaps == {i for i, item in enumerate(inputs) if item.get("diagnose_bindings")}
        assert seen_guided_states == {(i, step) for i, states in expected_guided.items() for step in range(len(states))}
        assert seen_environment_requirements == {i for i, item in enumerate(inputs) if item.get("environment_requirements")}
        assert set(exported_libraries) == set(library_exports)
        assert proved_code.execution_inputs_unchanged(hashes, report, external=[args.poly]), "Execution evidence changed."
        report.update(status="accepted", reasoning_cases=len(inputs), input_starts=starts, binding_source_executions=len(seen_views), findings=findings,
                      construction_frontier_executions=len(seen_frontiers), construction_seed_executions=len(seen_seeds),
                      coverage_executions=len(seen_coverage),
                      compilation_executions=sum("compilation_sources" in inputs[i] for i in seen_compilations),
                      native_library_executions=sum("native_libraries" in inputs[i] for i in seen_compilations),
                      binding_gap_executions=len(seen_binding_gaps),
                      environment_requirement_executions=len(seen_environment_requirements),
                      guided_state_executions=len(seen_guided_states),
                      native_guided_executions=sum("native_binding_guided" in item for item in inputs),
                      unproved_goal_assumption_cases=sum("unproved_goal_assumptions" in item for item in inputs),
                      exported_libraries=exported_libraries,
                      cases_source=str(Path(cases_source).resolve()) if cases_source is not None else str(Path(__file__).resolve()),
                      execution_inputs=hashes, results_sha256=digest(output / "results.log"),
                      proof_receipt=str(proof_path), proof_receipt_sha256=digest(proof_path),
                      boundary="Structural controls verify exact generation and conditional reports. Native semantic guarantees come from the separately accepted theory contracts; this host review is not native mathematical-proof admission.")
    except Exception as error:
        report["error"] = str(error)
        report["error_type"] = type(error).__name__
        report["error_traceback"] = traceback.format_exc()
    write_json(output / "receipt.json", report)
    print(json.dumps({k: v for k, v in report.items() if k not in {"execution_inputs", "findings", "evidence_archive"}}, indent=2))
    if report["status"] != "accepted":
        print((output / "results.log").read_text()[-5000:] if (output / "results.log").exists() else report["error"])
    return int(report["status"] != "accepted")


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--proof", type=Path, required=True)
    parser.add_argument("--poly", type=Path, required=True)
    parser.add_argument("--engine", type=Path, help="Retained generated module, with the exact digest in the proof receipt")
    parser.add_argument("--output", type=Path, required=True)
    return run(parser.parse_args())


if __name__ == "__main__":
    sys.exit(main())
