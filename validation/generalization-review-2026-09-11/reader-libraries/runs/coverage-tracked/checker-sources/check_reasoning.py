...
def reconstruct(item):
    item = {**item, "frontier": reconstruction_frontiers(item)[-1]}
...
def ml_schema(rule):
    ...
    return "N.make (" + ml_term(rule["head"], True) + ") (fs " + premises + ") (fs " + materials + ")"


def ml_library_source(entry):
    return "(n " + str(entry["entry"]) + ",(" + ml_schema(entry["schema"]) + "))"


def ml_library(entry):
    return "(n " + str(entry["entry"]) + ",(" + ml_schema(entry["schema"]) + "," + investigate.ml_list(entry["enumeration"], lambda r: ml_premise(r, True)) + "))"
...
def run(args, *, cases_factory=cases, cases_source=None, cases_dependencies=(), library_exports=()):
    ...
    report = {"status": "failed", "scope": "Complete finite reports for the supplied case family, with native comparisons where declared"}
    try:
        ...
        for i, item in enumerate(inputs):
            ...
            if operation and operation[0] == "binding":
                ...
                if operation[0] == "native_chain":
                    ...
                    if "compilation_sources" in item:
                        lib = "(N.natural_compiled_library " + investigate.ml_list(item["compilation_sources"], ml_library_source) + ")"
                        program += "val () = emitCompiledLibrary " + str(i) + " " + lib + ";\n"
                    frontier = investigate.ml_list(item["frontier"], ml_call)
                    call = ("N.natural_chained_investigation (n " + str(rounds) + ") " + lib + " " + frontier
            ...
            if item.get("diagnose_coverage"):
                coverage_call = ("N.reader_library_coverage" if item.get("native_reader_coverage") else
                                 "N.natural_library_coverage " + investigate.ml_list(item["library"], ml_library))
        ...
        for line in (output / "results.log").read_text().splitlines():
            if line.startswith("REASONING_RESULT "):
                ...
                index = int(identifier)
                assert 0 <= index < len(inputs) and index not in seen_compilations and "compilation_sources" in inputs[index]
                actual = json.loads(raw)
                expected = inputs[index]["library"]
                key = lambda e: (e["entry"], schema_key(e["schema"]), tuple(map(freeze, e["enumeration"])))
                assert list(map(key, actual)) == list(map(key, expected)), (inputs[index]["name"], "compiled library")
                ...
                assert normalized["blocked"] <= reconstruct(inputs[index])["residual"], (inputs[index]["name"], "unsound obstruction")
                ...
                expected = reconstruction_frontiers(inputs[index])
        ...
        assert seen_compilations == {i for i, item in enumerate(inputs) if "compilation_sources" in item}
        ...
        report.update(status="accepted", reasoning_cases=len(inputs), input_starts=starts, binding_source_executions=len(seen_views), findings=findings,
                      ...
                      compilation_executions=len(seen_compilations),
