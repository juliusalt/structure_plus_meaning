...
| "term_syntax (Payload_Term v) = payload_syntax v"
...
| "term_literal_bindings (Payload_Term v) = {}"
...
| "term_syntax_interior (Payload_Term v) = {[]}"
...
lemma term_literal_slots [simp]:
  ...
  "rel_dom (term_literal_bindings (Payload_Term v)) = {}"
...
lemma term_syntax_formed:
  ...
  using assms
  by (induction t) (auto intro: literal_syntax_formed payload_syntax_formed pair_syntax_formed)
...
next
  case (Payload_Term v)
  have ef: "environment_formed (term_environment (Payload_Term v))"
    by (rule term_environment_formed[OF Payload_Term.prems])
  have source: "artifact_at (term_environment (Payload_Term v)) None (payload_syntax v)"
    by (simp add: term_environment_def)
  have "term_quoted_at (term_environment (Payload_Term v)) None [] (Payload_Term v) {[]} {}"
    by (rule term_quoted_at.payload[OF ef source payload_syntax_recovers])
  then show ?case by (simp add: rel_dom_def)
next
  case (Pair_Term x y)
