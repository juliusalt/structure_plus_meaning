...
lemma payload_syntax_silent [simp]:
  "binder_silent (payload_syntax v)"
  by (auto simp: binder_silent_def payload_syntax_def headed_incidence_def
...
| "pattern_syntax f (Pattern_Payload v) = payload_syntax v"
...
| "pattern_literal_bindings (Pattern_Payload v) = {}"
...
| "pattern_syntax_interior (Pattern_Payload v) = {[]}"
...
lemma pattern_literal_slots [simp]:
  ...
  "rel_dom (pattern_literal_bindings (Pattern_Payload v)) = {}"
...
next
  ...
  then show ?case by (simp add: payload_syntax_formed)
next
...
lemma pattern_syntax_interior_finite [simp]: "finite (pattern_syntax_interior p)"
  by (induction p) auto

lemma pattern_syntax_interior_root [simp]: "[] \<in> pattern_syntax_interior p"
  by (cases p) (use literal_syntax_properties(2) in auto)
...
next
  case (Pattern_Payload v)
  show ?case by (simp add: rel_dom_def)
next
  case (Pattern_Pair p q)
  have paddr: "binder_addressing (pattern_variables p) f"
...
next
  case (Pattern_Payload v)
  have ef: "environment_formed (pattern_environment f (Pattern_Payload v))"
    by (rule pattern_environment_formed[OF Pattern_Payload.prems(1,2)])
  have source: "artifact_at (pattern_environment f (Pattern_Payload v)) None (payload_syntax v)"
    by (simp add: pattern_environment_def)
  have leaf: "term_quoted_at (pattern_environment f (Pattern_Payload v)) None [] (Payload_Term v) {[]} {}"
    by (rule term_quoted_at.payload[OF ef source payload_syntax_recovers])
  have separate: "{[]} \<inter> V = {}" using boundary by auto
  have quote: "pattern_quoted_at (pattern_environment f (Pattern_Payload v)) None V []
    (Pattern_Payload v) {[]} {}"
    by (rule pattern_quoted_at.payload[OF leaf separate])
  show ?case using quote by (simp add: rel_dom_def)
next
  case (Pattern_Pair p q)
...
lemma scoped_pattern_syntax_recovers:
  assumes formed: "pattern_formed p" and addressing: "binder_addressing (pattern_variables p) f"
    ...
    and source: "artifact_at E u (scope_wrapper (pattern_syntax f p) (f ` pattern_variables p) b r s t)"
    and literals: "\<forall>k\<in>rel_dom (pattern_literal_bindings p).
      external_slot_values E u k = {R. (k,R) \<in> pattern_literal_bindings p}"
  shows "scoped_pattern_at E u r (rename_pattern f p)
      (insert r (set [s,t] \<union> insert b (f ` pattern_variables p) \<union> pattern_syntax_interior p))
      (rel_dom (pattern_literal_bindings p)) \<and>
    rra_carrier (object_structure (scope_wrapper (pattern_syntax f p) (f ` pattern_variables p) b r s t)) =
      insert r (set [s,t] \<union> insert b (f ` pattern_variables p) \<union> pattern_syntax_interior p) \<union>
        rel_dom (pattern_literal_bindings p)"
proof -
  ...
  have art: "artifact_at E u ?T" by (rule source)
  ...
  have slots: "\<forall>k\<in>?K. external_slot_values (pattern_environment f p) None k = external_slot_values E u k"
    using literals by simp
  have body: "pattern_quoted_at E u ?V [] (rename_pattern f p) ?J ?K"
  ...
  have scoped: "scoped_pattern_at E u r (rename_pattern f p) ?I ?K"
  ...
  show ?thesis using scoped all_positions by blast
qed
...
proof -
  ...
  let ?I = "insert r (set [s,t] \<union> insert b ?V \<union> pattern_syntax_interior p)"
  have rf: "exact_formed ?R" by (rule pattern_syntax_formed[OF formed addressing])
  have carrier: "rra_carrier (object_structure ?R) =
    pattern_syntax_interior p \<union> ?K \<union> ?V" by (rule pattern_syntax_carrier[OF addressing])
  have inside: "?V \<subseteq> rra_carrier (object_structure ?R)" using carrier by blast
  have tf: "exact_formed ?T" by (rule scope_wrapper_formed[OF rf pattern_syntax_root inside addresses])
  ...
  have literal_values: "\<forall>k\<in>?K. external_slot_values ?E None k = {R. (k,R) \<in> pattern_literal_bindings p}"
    by simp
  have result: "scoped_pattern_at ?E None r (rename_pattern f p) ?I ?K \<and>
    rra_carrier (object_structure ?T) = ?I \<union> ?K"
    by (rule scoped_pattern_syntax_recovers[OF formed addressing distinct fresh addresses ef art literal_values])
  have closed: "environment_closed ?E {None} ((\<lambda>k. (None,k)) ` ?K)"
    by (rule literal_environment_closed[OF ef])
  show ?thesis
    by (rule exI[of _ ?E], rule exI[of _ ?I], rule exI[of _ ?K]) (use art result closed in blast)
qed
