...
proof -
  ...
  let ?Rc = "external_occurrence_syntax (snd d)"
  let ?Ci = "{[],[5]} :: local_address set"
  let ?Ck = "{[4]} :: local_address set"
  let ?Cs = "[{([4],d)},Cb,Cp]"
  let ?Ls = "[{},Lb,Lp]"
  let ?Is = "[?Ci,Ib,Ip]"
  let ?Ks = "[?Ck,Kb,Kp]"
  let ?Rs = "[?Rc,Rb,Rp]"
  have cf: "exact_formed ?Rc" by (rule external_occurrence_syntax_formed[OF address])
  have profile: "reference_table_formed {} {([4],d)}"
    by (rule reference_table_callees) (simp_all add: single_valued_def)
  have cc: "rra_carrier (object_structure ?Rc)=?Ci\<union>?Ck"
    by (auto simp: external_occurrence_syntax_def)
  have cs: "?Ci\<inter>?Ck={}" by auto
  have cd: "?Ck=rel_dom ({} :: (local_address\<times>exact_artifact) set)\<union>rel_dom {([4],d)}"
    by (auto simp: rel_dom_def)
  interpret assembled: record_syntax_construction ?Rs ?Is ?Ks ?Ls ?Cs
  proof (rule record_syntax_construction.intro)
    show "\<forall>R\<in>set ?Rs. exact_formed R" using cf b(1) p(1) by simp
    show "\<forall>R\<in>set ?Rs. bag_count (object_data R)=(\<lambda>_. 0)"
      using b(2) p(2) external_occurrence_syntax_properties(2) by simp
    show "\<forall>R\<in>set ?Rs. []\<in>rra_carrier (object_structure R)"
      using b(3) p(3) external_occurrence_syntax_properties(1) by simp
    show "length ?Is=length ?Rs" "length ?Ks=length ?Rs"
      "length ?Ls=length ?Rs" "length ?Cs=length ?Rs" by simp_all
    show "\<forall>i<length ?Rs. rra_carrier (object_structure (?Rs!i))=?Is!i\<union>?Ks!i"
      using cc b(5) p(5) by (auto simp: less_Suc_eq)
    show "\<forall>i<length ?Rs. ?Is!i\<inter>?Ks!i={}"
      using cs b(6) p(6) by (auto simp: less_Suc_eq)
    show "\<forall>i<length ?Rs. ?Ks!i=rel_dom (?Ls!i)\<union>rel_dom (?Cs!i)"
      using cd b(7) p(7) by (auto simp: less_Suc_eq)
    show "\<forall>i<length ?Rs. reference_table_formed (?Ls!i) (?Cs!i)"
      using profile b(4) p(4) by (auto simp: less_Suc_eq)
  qed
  have br: "rel_dom Lb\<union>rel_dom Cb\<subseteq>rra_carrier (object_structure Rb)" using b(5,7) by blast
  have pr: "rel_dom Lp\<union>rel_dom Cp\<subseteq>rra_carrier (object_structure Rp)" using p(5,7) by blast
  have bd: "rel_dom (fset V)\<subseteq>rel_ran Cb" using b(8) by simp
  have pd: "rel_dom D\<union>rel_ran D\<subseteq>rel_ran Cp" using p(8) by simp
  have cite_range: "rel_ran {([4],d)}={d}" by (auto simp: rel_ran_def)
  have deps: "rel_ran assembled.callees=insert d (rel_dom (fset V)\<union>rel_dom D\<union>rel_ran D)"
    using assembled.reference_range by (simp add: cite_range b(8) p(8) Un_assoc)
  have recover: "\<forall>E u. environment_formed E \<longrightarrow> artifact_at E u assembled.framed \<longrightarrow>
  ...
  proof (intro allI impI)
    ...
    have caddr: "finite_addressing (rra_carrier (object_structure ?Rc)) (syntax_branch 0)"
      using assembled.child_addressing[of 0] by simp
    have cread: "object_reads_agree (push_object (syntax_branch 0) ?Rc) assembled.framed
      (image (syntax_branch 0) (rra_carrier (object_structure ?Rc)))"
    ...
    have cite: "site_citation_at E u (syntax_branch 0 []) d (image (syntax_branch 0) ?Ci) {syntax_branch 0 [4]}"
    ...
    let ?C = "image (syntax_branch 0) ?Ci"
...
proof -
  ...
  show ?thesis by (rule exI[of _ frame.record_framed])
    (use frame.record_formed frame.record_counts carrier every in blast)
qed
