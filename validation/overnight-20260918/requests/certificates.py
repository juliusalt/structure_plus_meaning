...
val certificate_prepared = native_stage_timed "certificate_candidates" (fn () =>
  N.cause_certificate_prepare_family demands);
val () = marker "CERTIFICATE_PROFILES" (jlist jmatrix (map N.cause_certificate_profiles certificate_prepared));
val application_prepared = native_stage_timed "deferred_application_questions" (fn () =>
  N.cause_child_application_prepare_family []);
...
def assess(inputs, log):
    ...
    fields = ['CERTIFICATE_PROFILES','APPLICATION_PROFILES','QUESTION_AVAILABILITY',
    ...
    return {'physical_completion': True, **{k.lower(): block(k) for k in fields}, 'words': words,
            'timings': [dict(zip(('stage','wall','user','system'), x)) for x in re.findall(
                r'Physicalstage ([a-z_]+) wall=([0-9.]+) user=([0-9.]+) system=([0-9.]+)', text)],
            'scope': 'Complete independent certificate-construction review on every original full and leaf demand, retaining both original facets and all scope/report controls. Material-application questions are explicitly deferred after measured timeout, not discharged or replaced; source recovery/transport, native root replay and policy cause remain open.'}
...
r = isabelle_native_execution.checked_execution(proof, poly, output, workers=8, program=program,
    ...
    inputs={'originals': 'All body/adapter/target/root questions recomputed; root method consumed through original native admission.',
            ...
            'deferred_independent_question': 'Material-assisted application construction timed out separately; no verdict or method choice is inferred.',
            'criticism': ['reversal','duplicate candidates','each omitted facet','empty scope','absent report','wrong original-body report'],
            'certificates': 'Returned candidates checked by original finite_checks_schema_proof; no known calls or supplied satisfaction rows.'},
    assess=assess,
    question='Which available certificate constructions satisfy all original full and leaf child demands and the original proof checker? Independently scoped material-application questions remain unexecuted obligations.',
