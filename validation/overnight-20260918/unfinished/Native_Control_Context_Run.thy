theory Native_Control_Context_Run
...
text \<open>Every input entity is retained in the checked prefix. These summaries
  give its exact list coordinate and the complete old/new subject readings.\<close>

value [code] "(length development_seed_roots,length (fst development_seed_context),
  length (snd development_seed_context),fst development_seed_context,
  List.map_filter isabelle_head_constant development_seed_roots)"

value [code] "(let C=development_seed_context; D=context_run_changed in
  (C=D,snd C=snd D, map (\<lambda>i. (i,
    isabelle_declared_constant (snd C!i),
    isabelle_entity_subjects (fst C) (isabelle_development_constants (snd C)) (snd C!i),
    isabelle_entity_subjects (fst D) (isabelle_development_constants (snd D)) (snd D!i)))
    [0..<length (snd C)]))"

value [code] "Parallel.map (\<lambda>C. isabelle_context_assessment development_seed_roots C)
  [development_seed_context,context_run_changed]"

value [code] "(let C=development_seed_context; demands=development_seed_demands C in
  map_option (\<lambda>(accepted,refused). map (\<lambda>i. (i,demands!i |\<in>| accepted,
    demands!i |\<in>| refused)) [0..<length demands])
      (isabelle_demand_acceptance (snd C) demands))"
